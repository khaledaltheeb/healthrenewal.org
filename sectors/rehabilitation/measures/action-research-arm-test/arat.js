(()=>{
  ...
  function recalc(){
    const status=$('#arat-status')?.value||'testable';const statusBox=$('#arat-status-note');
    if(status!=='testable'){
      for(const sub of Object.keys(maxima))write(`#arat-${sub}-result`,'—');
      write('#arat-total','—');write('#arat-completion','الاختبار غير قابل للتفسير كدرجة معيارية في الحالة الحالية.');
    ...
    for(const [sub,max] of Object.entries(maxima)){
      const xs=valuesFor(sub),done=xs.filter(v=>v!==null).length;completed+=done;
      if(validScores(xs)){const sum=xs.reduce((a,b)=>a+b,0);total+=sum;write(`#arat-${sub}-result`,`${sum} / ${max}`);}else{allComplete=false;write(`#arat-${sub}-result`,`${done}/${xs.length} بنود مسجلة`);}
    }
    write('#arat-completion',`${completed}/19 بندًا مسجلًا.`);write('#arat-total',allComplete&&completed===19?`${total} / 57`:'—');
  }
  $$('.arat-score').forEach(el=>el.addEventListener('change',recalc));['#arat-status','#arat-arm','#arat-dominance'].forEach(s=>$(s)?.addEventListener('change',recalc));
  $('#arat-print')?.addEventListener('click',()=>window.print());
  $('#arat-clear')?.addEventListener('click',()=>{$$('input,textarea').forEach(el=>el.value='');$$('select').forEach(el=>el.selectedIndex=0);recalc();const live=$('#arat-live');if(live)live.textContent='تم مسح ورقة ARAT محليًا.';});
  recalc();
})();
