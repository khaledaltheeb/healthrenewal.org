...
const number=(id)=>{const el=document.getElementById(id);if(!el||String(el.value).trim()==='')return null;const v=Number(el.value);return Number.isFinite(v)?v:null};
const set=(id,text)=>{const el=document.getElementById(id);if(el)el.textContent=text};
const fmt=(n,d=2)=>Number.isFinite(n)?n.toFixed(d):'—';
function announce(text){const el=$('#live');if(el)el.textContent=text}
function calc10m(){const recordedDistance=number('walk-distance');const distance=recordedDistance!==null&&recordedDistance>0?recordedDistance:10;const t1=number('walk-t1'),t2=number('walk-t2');const vals=[t1,t2].filter(v=>v!==null&&v>0);if(!vals.length){set('walk-result','—');return}const mean=vals.reduce((a,b)=>a+b,0)/vals.length;const speed=distance/mean;set('walk-result',`${fmt(speed,2)} م/ث · متوسط الزمن ${fmt(mean,2)} ث`)}
function calcPSFS(){const vals=$$('.psfs-score').map(el=>String(el.value).trim()===''?null:Number(el.value)).filter(v=>v!==null&&Number.isFinite(v));if(!vals.length){set('psfs-result','—');return}set('psfs-result',`${fmt(vals.reduce((a,b)=>a+b,0)/vals.length,2)} / 10 (${vals.length} أنشطة)`)}
...
function calcTUG(){const vals=[number('tug-t1'),number('tug-t2')].filter(v=>v!==null&&v>0);if(!vals.length){set('tug-result','—');return}const best=Math.min(...vals);const mean=vals.reduce((a,b)=>a+b,0)/vals.length;set('tug-result',`أفضل زمن ${fmt(best,2)} ث · المتوسط ${fmt(mean,2)} ث`)}
function calcFTSST(){const t=number('fts-time');set('fts-result',t!==null&&t>0?`${fmt(t,2)} ثانية`:'—')}
...
function calcBalance(){const rows=['side','semi','tandem','single'];const vals=rows.map(k=>number(`bal-${k}`)).filter(v=>v!==null);if(!vals.length){set('balance-result','—');return}const min=Math.min(...vals);const completed=vals.filter(v=>v>=10).length;set('balance-result',`${completed}/${vals.length} وضعيات مسجلة بلغت 10 ثوانٍ · أقل زمن ${fmt(min,1)} ث`)}
