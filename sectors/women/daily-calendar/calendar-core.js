(() => {
  ...
  const factBank = [
    "تسجيل بداية النزف والألم والمزاج والنوم عبر عدة مرات حيض يعطي وصفًا أفضل من الاعتماد على الذاكرة وحدها.",
    ...
    "عدم انتظام الحيض قد يحدث لأسباب متعددة؛ التطبيق لا يستطيع تحديد السبب بمفرده.",
  ...
  const tipBank = [
    ...
    "لا تستخدمي توقع الحيض لتأكيد أو نفي الحمل؛ استخدمي اختبارًا مناسبًا واستشارة عند الحاجة.",
  ...
  const ideaBank = [
    ...
    "استخدمي لونًا لتوقع الحيض ولونًا مختلفًا للتاريخ الفعلي كي تلاحظي دقة التوقع.",
  ...
  const suggestionBank = [
    ...
    "سجلي تاريخ بدء الحيض الفعلي عند حدوثه كي تتحسن حسابات المتوسط لديك.",
  ...
  const tenMinuteBank = [
    ...
    "حدّثي تاريخ بدء الحيض الفعلي ودوّني فرق التوقع دون لوم أو قلق.",
  ...
  function renderToday() {
    ...
    if (info) $("cycleChip").textContent = `اليوم التقديري ${Math.max(1, info.cycleDay)} من الحيض`;
    renderCycleSummary();
    renderStreak();
  }
  ...
  function eventTitle(kind = "daily") {
    if (kind === "period") return state.settings.privacyMode === "explicit" ? "نافذة الحيض المتوقعة" : "متابعة شخصية";
  ...
  function bindEvents() {
    ...
    $("settingsForm").addEventListener("submit", (event) => {
      ...
      if (next.cycleEnabled && !parseIso(next.lastPeriodStart)) { $("settingsStatus").textContent = "أضيفي أول يوم لآخر حيض أو ألغِ تفعيل التوقع."; $("lastPeriodStart").focus(); return; }
      state.settings = next;
      saveState();
    ...
    $("clearPrivateData").addEventListener("click", () => {
      if (!window.confirm("سيُحذف سجل الحيض والملاحظات والإنجازات والإعدادات من هذا المتصفح فقط. هل تتابعين؟")) return;
    ...
    $("markPeriodStart").addEventListener("click", () => {
      ...
      $("logStatus").textContent = "سُجلت بداية الحيض لهذا اليوم وأُعيد حساب التوقع محليًا.";
    });
