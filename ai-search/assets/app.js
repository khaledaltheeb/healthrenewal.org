...
function resultCard(result, index) {
  ...
  source.className = 'result-link';
  source.href = result.url;
  appendText(source, 'فتح الصفحة الأصلية ←');
...
function render(payload) {
  ...
  if (!results.length) {
    ...
    appendText(empty, 'لم تظهر نتائج كافية. جرّب صياغة أقصر أو اسم الحالة أو الموضوع مباشرة.');
    resultsNode.append(empty);
    summaryNode.textContent = `لم تُعثر نتائج مناسبة باستخدام ${label}.`;
    return;
  }
  ...
  summaryNode.textContent = `${results.length} نتيجة مرتبة باستخدام ${label}.`;
}
...
function submitSearch() {
  ...
  setStatus(semanticToggle.checked
    ? (indexMode === 'local-rerank'
      ? 'يجري فهم السؤال، واختيار الصفحات المرشحة، ثم إعادة ترتيب محتواها…'
      : 'يجري فهم السؤال وترتيب المقاطع…')
...
worker.addEventListener('message', (event) => {
  ...
  if (message.type === 'index-progress') {
    setStatus(message.message || 'جارٍ تجهيز فهرس البحث…');
    return;
  }
  ...
  if (message.type === 'ready') {
    ready = true;
    indexMode = message.indexMode || 'local-rerank';
    ...
    setStatus(indexMode === 'generated'
      ...
      : `اكتُشفت ${count} صفحة. سيطبّق E5 على مجموعة مرشحة محدودة بدل فهرسة كل الصفحات على جهازك.`, 'ready');
