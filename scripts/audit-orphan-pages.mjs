...
const SITE_BASE = normalizeBase(process.env.SITE_BASE_PATH ?? '/pterminology-site');
...
function normalizeInternalHref(href, sourcePage) {
  ...
  try {
    ...
    pathname = stripSiteBase(pathname);
...
async function main() {
  ...
  while (queue.length) {
    ...
    for (const target of pages.get(source)?.links ?? []) {
      if (pages.has(target) && !reachable.has(target)) { reachable.add(target); queue.push(target); }
    }
  }
  const orphans = [...pages.keys()].filter((page) => page !== '/' && !reachable.has(page)).sort((a, b) => a.localeCompare(b, 'ar'));
  const report = {
    generatedAt: new Date().toISOString(),
    siteBasePath: SITE_BASE || '/',
  ...
  const markdown = [
    '# Orphan page audit', '',
    `- Site base path: \`${report.siteBasePath}\``,
    ...
    ...(brokenInternalTargets.length ? brokenInternalTargets.map(({ source, target }) => `- \`${source}\` → \`${target}\``) : ['None.']), '',
  ].join('\n');
  await fs.writeFile(path.join(REPORT_DIR, 'orphan-pages.md'), markdown);
  console.log(markdown);
  if (STRICT && (orphans.length || brokenInternalTargets.length)) process.exitCode = 1;
}
