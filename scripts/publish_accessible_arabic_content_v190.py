...
def load_content() -> dict:
    ...
    required = {
        "slug", "title", "description", "summary", "scope_limits", "sections",
        "checklist", "examples", "internal_links", "sources", "reviewed_at", "status",
    }
...
def render(data: dict) -> str:
    ...
    article = {
        ...
        "about": [
            {"@type": "Thing", "name": "إتاحة الويب"},
            {"@type": "Thing", "name": "المحتوى العربي الرقمي"},
    ...
    sources = "".join(
        f'<li><a rel="noopener" href="{esc(item["url"])}">{esc(item["title"])}</a> — {esc(item["publisher"])}</li>'
...
<header><h1>{esc(data["title"])}</h1><p class="lead">{esc(data["summary"])}</p><p class="notice"><strong>حدود الاستخدام:</strong> {esc(data["scope_limits"])}</p></header>
{sections}
<section class="checklist"><h2>قائمة مراجعة قبل النشر</h2><ol>{checklist}</ol></section>
<section class="examples"><h2>أمثلة تحريرية سريعة</h2><table><thead><tr><th>صياغة تحتاج تحسينًا</th><th>بديل أوضح</th></tr></thead><tbody>{examples}</tbody></table></section>
<section><h2>مسارات مرتبطة</h2><ul>{links}</ul></section>
<section><h2>المصادر وحدود المراجعة</h2><p>الحالة التحريرية: مراجعة داخلية، ولا تمثل شهادة امتثال أو مراجعة خارجية. آخر مراجعة: {esc(data["reviewed_at"])}.</p><ul>{sources}</ul></section>
</main></body></html>'''
...
def add_contextual_link(path: Path, href: str, label: str) -> None:
    ...
    if href in text:
        return
    link = f'<p class="accessible-content-link"><a href="{href}">{esc(label)}</a></p>'
...
def publish(site: Path) -> Path:
    ...
    add_contextual_link(site / "special-needs" / "index.html", href, "دليل إتاحة المحتوى العربي الرقمي")
    add_contextual_link(site / "audiences" / "teacher" / "index.html", href, "قائمة عملية لإتاحة المحتوى التعليمي الرقمي")
    add_contextual_link(site / "audiences" / "family" / "index.html", href, "كيف تنشر محتوى عربيًا أكثر إتاحة؟")
    ...
    report = site / "api" / "accessible-arabic-digital-content-v190.json"
