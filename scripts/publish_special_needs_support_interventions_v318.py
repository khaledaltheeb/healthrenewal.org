...
CONTENT = ROOT / "content" / "v318" / "special-needs-support-interventions-ar.json"
BASE = "https://healthrenewal.org"
BP = "/"
VERSION = 318
MARKER = "data-support-interventions-v318"
...
def validate_payload(payload: dict) -> list[dict]:
    if payload.get("version") != VERSION or payload.get("language") != "ar":
        raise SystemExit("Support intervention manifest contract failed")
    if payload.get("review_status") != "internally-reviewed-external-clinical-review-required":
        raise SystemExit("Support intervention review status must remain honest")
    guides = payload.get("guides")
    if not isinstance(guides, list) or len(guides) != 2:
        raise SystemExit("Exactly two support intervention guides are required")
    if {guide.get("slug") for guide in guides} != {
        ...
        raise SystemExit("Support intervention routes are incomplete")
    if {guide.get("parent_slug") for guide in guides} != {"autism", "down-syndrome"}:
        raise SystemExit("Support intervention parent routes are incomplete")
    ...
    for guide in guides:
        ...
        if not isinstance(sections, list) or len(sections) != 5:
            raise SystemExit(f"Each support guide must contain five sections: {guide.get('slug')}")
        if not isinstance(sources, list) or len(sources) < 4:
            raise SystemExit(f"Each support guide must contain at least four sources: {guide.get('slug')}")
        if len(guide.get("plan_steps", [])) < 5 or len(guide.get("urgent", [])) < 3:
            raise SystemExit(f"Plan or safety depth failed: {guide.get('slug')}")
...
def render_guide(guide: dict, payload: dict) -> str:
    ...
    for section in guide["sections"]:
        ...
        sections.append(
            f'<section class="section-card" id="{e(section["id"])}"><p class="kicker">محور دعم</p>'
            f'<h2>{e(section["title"])}</h2><p>{e(section["summary"])}</p><ul>{points}</ul><p>{refs}</p></section>'
        )
    plan = "".join(f"<li>{e(item)}</li>" for item in guide["plan_steps"])
...
def patch_parent(site: Path, guide: dict) -> None:
    ...
    pattern = rf'<section class="section" {MARKER} data-support-guide="{re.escape(guide["slug"])}">.*?</section>'
    if re.search(pattern, text, flags=re.S):
        ...
        count = 1
    if count != 1 or text.count(f'data-support-guide="{guide["slug"]}"') != 1:
        raise SystemExit(f"Parent support-guide idempotence failed: {path}")
...
def publish(site: Path) -> dict:
    ...
    for guide in guides:
        ...
        if page.count("<h1") != 1 or page.count('class="section-card"') != 5 or BANNED.search(page):
            raise SystemExit(f"Rendered support-guide contract failed: {target}")
    ...
    report = {
        ...
        "plan_step_count": sum(len(guide["plan_steps"]) for guide in guides),
    ...
    (site / "api" / "special-needs-support-interventions-v318.json").write_text(
