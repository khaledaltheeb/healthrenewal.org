...
STYLE = r'''<style id="institutional-header-v232-styles">
...
@media(max-width:900px){.institutional-header .header-shell{display:grid;grid-template-columns:1fr;gap:9px;padding-block:9px 12px}.institutional-header .institutional-brand{justify-content:center;text-align:center}.institutional-nav{display:grid;grid-template-columns:minmax(0,1fr) minmax(150px,.56fr);width:100%;gap:8px}.header-menu>summary{width:100%}.header-menu-panel{inset-inline:0;width:100%;max-height:min(66vh,540px);overflow:auto}.sections-list{grid-template-columns:1fr}.language-panel{width:100%}}
@media(max-width:520px){.institutional-header .institutional-brand img{width:46px;height:46px}.institutional-header .brand-copy strong{font-size:.9rem}.institutional-header .brand-copy small{font-size:.7rem}.institutional-nav{grid-template-columns:1fr}.header-menu-panel{position:relative;inset:auto;margin-block-start:8px;box-shadow:0 12px 32px rgba(14,63,67,.14)}}
...
def render_header() -> str:
    ...
    language_links = "".join(
        f'<li><a lang="{language}" dir="{direction}" href="{esc(href)}"'
        f'{" aria-current=\"page\"" if current else ""}>{esc(label)}</a></li>'
        for label, href, language, direction, current in LANGUAGE_LINKS
    )
    return f'''<header class="institutional-header" aria-label="الترويسة الرئيسية" data-institutional-header-v232>
  <div class="wrap header-shell">
    ...
    <nav class="institutional-nav" aria-label="التنقل الرئيسي">
      <details class="header-menu sections-menu"><summary><span>أقسام المنصة</span><span class="menu-caret" aria-hidden="true">⌄</span></summary><div class="header-menu-panel sections-panel"><p class="menu-heading">انتقل مباشرة إلى القسم الذي تحتاجه</p><ul class="sections-list">{section_links}</ul></div></details>
      <details class="header-menu language-menu" data-i18n-switcher-v72><summary><span class="language-summary"><small>اللغة</small><strong>العربية</strong></span><span class="menu-caret" aria-hidden="true">⌄</span></summary><div class="header-menu-panel language-panel"><p class="menu-heading">اختر لغة الواجهة</p><ul class="language-list">{language_links}</ul></div></details>
    </nav>
  </div>
</header>'''
...
def header_pattern() -> re.Pattern[str]:
    return re.compile(r'<header\b[^>]*aria-label="الترويسة الرئيسية"[^>]*>.*?</header>', re.S)
...
def validate(text: str) -> dict[str, object]:
    ...
    header = matches[0]
    errors: list[str] = []
    if 'data-institutional-header-v232' not in header:
    ...
    for label, route in SECTION_LINKS:
        if label not in header or f'href="{BASE_PATH}{route}"' not in header:
            errors.append(f"missing section link: {label}")
    ...
    for marker in ('id="institutional-header-v232-styles"', '@media(max-width:900px)', '@media(max-width:520px)', '@media(prefers-reduced-motion:reduce)'):
        if marker not in text:
            errors.append(f"missing style marker: {marker}")
    if errors:
        raise SystemExit("Institutional header v232 validation failed:\n" + "\n".join(errors))
    return {"version": VERSION, "status": "passed", "section_links": 12, "language_links": 3, "dropdowns": 2, "responsive": True, "keyboard_native_details": True, "reduced_motion_supported": True}
...
def publish(site: Path | str = SITE) -> dict[str, object]:
    ...
    if 'id="institutional-header-v232-styles"' not in after:
    ...
    report["before_sha256"] = hashlib.sha256(before.encode()).hexdigest()
    report["after_sha256"] = hashlib.sha256(after.encode()).hexdigest()
    index.write_text(after, encoding="utf-8")
    api = target / "api"
    api.mkdir(parents=True, exist_ok=True)
    (api / "institutional-header-v232.json").write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return report
