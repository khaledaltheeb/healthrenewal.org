...
import care_guides_wave_v400 as previous_wave
from care_guides_topics_v401_1a import TOPICS as TOPICS_1A
from care_guides_topics_v401_1b import TOPICS as TOPICS_1B
from care_guides_topics_v401_1c import TOPICS as TOPICS_1C
from care_guides_topics_v401_2 import TOPICS as TOPICS_2
VERSION = 401
RELEASE_DATE = "2026-08-14"
WAVE = 2
EXPECTED_WAVE_GUIDES = 50
EXPECTED_INSTITUTIONAL_GUIDES = 187
EXPECTED_SOURCE_GUIDES = 201
MINIMUM_PUBLISHED_GUIDES = 200
TARGET_TOTAL_PAGES = 10000

def topics() -> list[tuple[str, ...]]:
    values = [*TOPICS_1A, *TOPICS_1B, *TOPICS_1C, *TOPICS_2]
    ...
    if len(slugs) != len(set(slugs)) or len(titles) != len(set(titles)):
        raise RuntimeError("Wave 002 contains duplicate slugs or titles")
    return values

def install(implementation: object) -> dict[str, object]:
    if getattr(implementation, "_care_guides_wave_v401_installed", False):
        return getattr(implementation, "_care_guides_wave_v401_report")
    ...
    if collisions or title_collisions:
        raise RuntimeError(f"Wave 002 collides with existing guides: {collisions} {title_collisions}")
    ...
    if len(wave_guides) != EXPECTED_WAVE_GUIDES:
        raise RuntimeError("Wave 002 did not generate exactly 50 guides")
    report = {
        "version": VERSION,
        "wave": WAVE,
        "target_total_pages": TARGET_TOTAL_PAGES,
        "added_guides": EXPECTED_WAVE_GUIDES,
        "cumulative_wave_guides": previous_wave.EXPECTED_WAVE_GUIDES + EXPECTED_WAVE_GUIDES,
    ...
    implementation._care_guides_wave_v401_installed = True
    implementation._care_guides_wave_v401_report = report
    return report
