...
def infer_clusters(text: str, taxonomy: dict[str, Any]) -> list[str]:
    ...
    for cluster in taxonomy.get("clusters") or []:
        terms = list(cluster.get("primary_terms") or []) + list(cluster.get("synonyms") or [])
        score = sum(2 if term in cluster.get("primary_terms", []) else 1 for term in terms if str(term).casefold() in normalized)
        if score:
...
def audit_page(path: Path, site: Path, taxonomy: dict[str, Any]) -> dict[str, Any]:
    ...
    cluster_text = " ".join([parser.title, parser.description, visible[:12000]])
    return {
        "path": relative,
        "title": parser.title,
        ...
        "clusters": infer_clusters(cluster_text, taxonomy),
        "warnings": warnings,
        "critical": critical,
    }
...
def audit_site(site: Path = SITE, report_path: Path = REPORT_PATH, taxonomy_path: Path = TAXONOMY_PATH) -> dict[str, Any]:
    ...
    duplicate_titles = {key: value for key, value in title_paths.items() if len(value) > 1}
    duplicate_descriptions = {key: value for key, value in description_paths.items() if len(value) > 1}
