...
import publish_mowat_wilson_kleefstra_v326 as previous
import publish_new_special_needs_conditions_v323 as render_base

ROOT = Path(__file__).resolve().parents[1]
CONTENT = ROOT / "content" / "v327" / "phelan-mcdermid-satb2-guides-ar.json"
VERSION = 327
...
def read_payload() -> dict:
    try:
        ...
        raise SystemExit(f"Invalid v327 JSON: {exc}") from exc
    if not isinstance(data, dict):
        raise SystemExit("v327 content must be an object")
    return data
...
def validate_payload(data: dict) -> list[dict]:
    ...
    for guide in guides:
        ...
        if any(not str(guide.get(key, "")).strip() for key in required):
            raise SystemExit(f"Incomplete v327 guide identity: {guide.get('slug')}")
        if len(guide.get("key_facts", [])) < 5:
            raise SystemExit(f"Insufficient v327 key facts: {guide['slug']}")
        sections = guide.get("sections")
        sources = guide.get("sources")
        if not isinstance(sections, list) or len(sections) != 7:
            raise SystemExit(f"Each v327 condition needs seven sections: {guide['slug']}")
        if not isinstance(sources, list) or len(sources) != 7:
            raise SystemExit(f"Each v327 condition needs exactly seven sources: {guide['slug']}")
        if len(guide.get("action_steps", [])) < 8 or len(guide.get("urgent", [])) < 6:
            raise SystemExit(f"v327 action or urgent depth failed: {guide['slug']}")
        if len(guide.get("myths", [])) < 5 or len(guide.get("faqs", [])) < 5:
            raise SystemExit(f"v327 myth or FAQ depth failed: {guide['slug']}")
        ...
        for source in sources:
            ...
            if not sid or sid in source_index:
                raise SystemExit(f"Duplicate v327 source id in {guide['slug']}: {sid}")
            if not is_https(url) or url in urls:
                raise SystemExit(f"Invalid or duplicate v327 source URL in {guide['slug']}: {url}")
            if source.get("level") not in {"S1", "S2", "S3", "S4", "S5"}:
                raise SystemExit(f"Invalid v327 source level: {guide['slug']}/{sid}")
            if any(not str(source.get(key, "")).strip() for key in ("organization", "title", "reviewed")):
                raise SystemExit(f"Incomplete v327 source: {guide['slug']}/{sid}")
        ...
        for section in sections:
            ...
            if not section_id or section_id in section_ids:
                raise SystemExit(f"Invalid v327 section id: {guide['slug']}/{section_id}")
            if not str(section.get("title", "")).strip() or not str(section.get("summary", "")).strip():
                raise SystemExit(f"Incomplete v327 section: {guide['slug']}/{section_id}")
            if len(section.get("points", [])) < 7 or not source_ids:
                raise SystemExit(f"v327 section depth failed: {guide['slug']}/{section_id}")
            if any(source_id not in source_index for source_id in source_ids):
                raise SystemExit(f"Unknown v327 source reference: {guide['slug']}/{section_id}")
            section_ids.add(section_id)
            used.update(source_ids)
        if set(source_index) - used:
            raise SystemExit(f"Unused v327 sources in {guide['slug']}: {sorted(set(source_index) - used)}")
    return guides
...
def combined_payload(extension: dict) -> tuple[dict, list[dict], list[dict]]:
    ...
    if len(slugs) != len(set(slugs)):
        raise SystemExit("v327 duplicates an existing condition route")
...
def render_expanded_cluster(data: dict, guides: list[dict]) -> str:
    ...
    page, count = re.subn(
        ...
        '<section class="wrap panel"><h2>منهج التوسع التالي</h2><p>يضم المركز الآن أحد عشر دليلًا مستقلًا، بعد إضافة فيلان–ماكديرميد المرتبطة بـSHANK3 والمتلازمة المرتبطة بـSATB2 إلى الحالات التسع السابقة. تستمر الإضافة بعد فحص عدم التكرار وربط المحاور بالمصادر واختبار العمق والفهرسة وحالة المراجعة.</p></section>',
        page,
        count=1,
        flags=re.S,
    )
    if count != 1:
        raise SystemExit("v327 could not replace the nine-condition cluster roadmap")
    return page
...
def publish(site: Path) -> dict:
    ...
    if missing_previous:
        raise SystemExit(f"v327 requires v323-v326 production layers first: {missing_previous}")
    ...
    for guide in new_guides:
        ...
        if words < 1750:
            raise SystemExit(f"v327 condition page is too shallow: {guide['slug']}={words}")
        if page.lower().count("<h1") != 1 or page.count('class="section-card"') != 7:
            raise SystemExit(f"v327 rendered structure failed: {guide['slug']}")
        if "MedicalWebPage" not in page or "FAQPage" not in page or render_base.BANNED.search(page):
            raise SystemExit(f"v327 rendered metadata or language failed: {guide['slug']}")
    ...
    (api / "phelan-mcdermid-satb2-guides-v327.json").write_text(
