...
import care_guides_wave_v406 as previous_wave
from care_guides_sources_v246 import SOURCES
from care_guides_topics_v407 import TOPICS as RAW_TOPICS

VERSION=407
RELEASE_DATE="2026-08-14"
WAVE=8
EXPECTED_WAVE_GUIDES=50
EXPECTED_INSTITUTIONAL_GUIDES=487
EXPECTED_SOURCE_GUIDES=501
MINIMUM_PUBLISHED_GUIDES=500
TARGET_TOTAL_PAGES=10000
EXCLUDED_SLUGS={"services-first-call-intake-prep","services-waitlist-tracker","services-complaint-facts-log","services-document-expiry-calendar","services-case-number-master-list"}
CATEGORY_MAP={"developmental":"education","learning":"education","access":"access","communication":"family","services":"professionals","rights":"professionals","work":"work","caregiving":"caregivers","older":"older-adults","medication":"professionals","chronic":"self-management","pain":"self-management","sleep":"self-management","anxiety":"self-management"}
...
def topics():
    ...
    if len({x[0] for x in values})!=50 or len({x[1] for x in values})!=50: raise RuntimeError("Wave 008 contains duplicate slugs or titles")
    missing_sources=sorted({x[8] for x in values if x[8] not in SOURCES})
    if missing_sources: raise RuntimeError(f"Wave 008 uses unsupported source keys: {missing_sources}")
    return values
...
def install(implementation):
    if getattr(implementation,"_care_guides_wave_v407_installed",False): return implementation._care_guides_wave_v407_report
    ...
    if collisions or title_collisions: raise RuntimeError(f"Wave 008 collides with existing guides: {collisions} {title_collisions}")
