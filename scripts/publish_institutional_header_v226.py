...
STYLE = r'''<style id="institutional-header-v226-styles">
...
def render_header() -> str:
    return f'''<header class="institutional-header" aria-label="الترويسة الرئيسية" data-institutional-header-v226>
...
def validate(text: str) -> dict[str, object]:
    ...
    header = matches[0]
    errors: list[str] = []
    if 'data-institutional-header-v226' not in header:
    ...
    for marker in (
        'id="institutional-header-v226-styles"',
    ...
    if errors:
        raise SystemExit("Institutional header v226 validation failed:\n" + "\n".join(errors))
    return {
        "version": VERSION,
        "status": "passed",
...
def publish(site: Path | str = SITE) -> dict[str, object]:
    ...
    if 'id="institutional-header-v226-styles"' not in after:
    ...
    (api / "institutional-header-v226.json").write_text(
