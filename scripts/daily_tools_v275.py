...
def source_list(data: dict[str, Any], source_ids: Iterable[str]) -> str:
    ...
    return "<ul>" + "".join(
        ...
        f'{e(sources[source_id]["publisher"])} — {e(sources[source_id]["title"])} '
        f'({e(sources[source_id]["year"])})</a><br>'
...
def _index_cards(data: dict[str, Any]) -> str:
    ...
    return "".join(
        f'<article data-tool-card data-category="{e(tool["category"])}" '
        f'data-search="{e(" ".join([tool["title"], tool["intent"], categories[tool["category"]]["title"], *tool["audience"]]))}">'
...
<div class="summary-strip"><div><strong>{len(data["tools"])}</strong> أداة</div>
<div><strong>{len(data["categories"])}</strong> مجالات</div>
<div><strong>{len(data["sources"])}</strong> مصدرًا مؤسسيًا</div></div>
<div class="filters"><label>البحث في العنوان والغرض والجمهور
<input type="search" data-tool-search placeholder="مثال: النوم، الأسرة، المدرسة، الحدود" autocomplete="off"></label>
<label>المجال<select data-tool-category><option value="">كل المجالات</option>{options}</select></label></div>
...
(() => {
  ...
  const apply = () => {
    ...
    let count = 0;
    cards.forEach(card => {
      const visible = (!q || normalize(card.dataset.search).includes(q)) && (!c || card.dataset.category === c);
      card.hidden = !visible;
      if (visible) count += 1;
    });
...
[data-tool-card][hidden]{display:none!important}
...
def enhance_index(data: dict[str, Any], site: Path) -> None:
    ...
    text = text.replace("</style>", EXTRA_STYLE + "</style>", 1)
    pattern = r'(<header>.*?</header>)(<div class="grid">.*?</div>)(<section><h2>المصادر</h2>)'
    replacement = (
        r"\1" + _search_block(data) + '<div class="grid" data-tool-grid>'
        + _index_cards(data) + "</div>" + SEARCH_SCRIPT + r"\3"
    )
    ...
    def transform(payload: dict[str, Any]) -> dict[str, Any]:
        payload["name"] = "100 أداة نفسية وتنظيمية يومية"
        payload["description"] = data["description"]
        payload["dateModified"] = data["reviewed_at"]
        payload["hasPart"] = [
            {
                "@type": "WebApplication",
                "position": tool["id"],
                ...
                "url": BASE + "daily-tools/" + tool["slug"] + "/",
            }
            for tool in data["tools"]
        ]
        return payload

    text = _replace_jsonld(text, transform)
...
def enhance_tool(data: dict[str, Any], tool: dict[str, Any], site: Path) -> None:
    ...
    header_extra = (
        f'<p><span class="category-pill">{e(category["title"])}</span></p>'
        f'<p><strong>مناسبة لـ:</strong> {e("، ".join(tool["audience"]))} — '
        f'<strong>آخر مراجعة:</strong> {e(tool["reviewed_at"])}</p>'
    )
    ...
    rich_steps = (
        f'<section><h2>متى تفيد هذه الأداة؟</h2>{list_html(tool["when_to_use"])}</section>'
        f'<section><h2>خطوات الاستخدام المنهجي</h2>{list_html(tool["steps"], ordered=True)}</section>'
        f'<section><h2>أسئلة المراجعة</h2>{list_html(tool["review_questions"])}</section>'
    )
    text, count = re.subn(
        r'<section><h2>الخطوات</h2><ol>.*?</ol></section>',
        rich_steps,
        text,
        count=1,
        flags=re.S,
    ...
    after_form = (
        ...
        f'<section><h2>كيف تقرأ ما سجلته؟</h2>{list_html(tool["interpretation"])}</section>'
        f'<section><h2>ما الخطوة التالية؟</h2>{list_html(tool["next_steps"])}</section>'
        '</div>'
        f'<section class="note"><h2>ما يجب تجنبه</h2>{list_html(tool["avoid"])}</section>'
    )
    text = text.replace(
        '<section class="note"><h2>السلامة</h2>',
        after_form + '<section class="note"><h2>السلامة ومتى تطلب المساعدة</h2>',
        1,
    )

    related = "".join(
        f'<li><a href="{PATH}daily-tools/{e(slug)}/">'
        f'{e(next(item["title"] for item in data["tools"] if item["slug"] == slug))}</a></li>'
        for slug in tool["related_tools"]
    )
    evidence = (
        '<section><h2>الأساس المنهجي وحدوده</h2>'
        f'<p>{e(tool["evidence_note"])}</p>'
        f'<p><strong>تاريخ التحقق والمراجعة:</strong> {e(tool["reviewed_at"])}</p>'
        f'{source_list(data, tool["source_ids"])}</section>'
        f'<section><h2>أدوات مرتبطة</h2><ul>{related}</ul></section>'
    )
    text, count = re.subn(
        r'<section><h2>مصادر المنهج</h2>.*?</section>',
        evidence,
        text,
        count=1,
        flags=re.S,
    ...
    def transform(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for item in graph:
            if item.get("@type") == "WebApplication":
                item["dateModified"] = tool["reviewed_at"]
                item["audience"] = {"@type": "Audience", "audienceType": "، ".join(tool["audience"])}
                item["about"] = category["title"]
                item["citation"] = [
                    next(source["url"] for source in data["sources"] if source["id"] == source_id)
                ...
                "mainEntity": [
                    {
                        "@type": "Question",
                        "name": question,
                        "acceptedAnswer": {"@type": "Answer", "text": tool["interpretation"][index]},
                    }
                    for index, question in enumerate(tool["review_questions"])
                ],
            }
        )
        payload["@graph"] = graph
...
def patch_homepage(data: dict[str, Any], site: Path) -> None:
    ...
    text = path.read_text(encoding="utf-8")
    card = (
        '<article class="card" data-daily-tools-v219><h3>100 أداة نفسية وتنظيمية يومية</h3>'
        '<p>أدوات عربية محلية للتنظيم والمتابعة والتواصل والأسرة والمدرسة والإتاحة وطلب المساعدة، '
        'مع حدود غير تشخيصية ومصادر أصلية.</p><a href="daily-tools/">فتح مركز الأدوات</a></article>'
    )
    if "data-daily-tools-v219" in text:
        text = re.sub(
            r'<article class="card" data-daily-tools-v219>.*?</article>',
            card,
            text,
            count=1,
            flags=re.S,
        )
    text = text.replace("ثماني أدوات نفسية تفاعلية", "100 أداة نفسية وتنظيمية")
    text = text.replace("أدوات نفسية تفاعلية يومية", "100 أداة نفسية وتنظيمية يومية")
    path.write_text(text, encoding="utf-8")
    ...
    report.update(
        {
            ...
            "content_contract": data["content_contract"],
            "tools": len(data["tools"]),
            "categories": len(data["categories"]),
            "paths": len(data["paths"]),
            "pages": len(data["tools"]) + len(data["paths"]) + 2,
