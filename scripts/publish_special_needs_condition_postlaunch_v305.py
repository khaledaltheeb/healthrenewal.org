...
def load_config() -> dict[str, Any]:
    ...
    for slug in SLUGS:
        ...
        all_hrefs.extend(str(link.get("href", "")) for link in item["related_guides"])
    all_hrefs.extend(str(link.get("href", "")) for link in common)
    ...
    if len(all_hrefs) != len(set(all_hrefs)) + 2:
        # The early-intervention route is intentionally reused once across both conditions;
        # other duplicated routes are rejected below by per-page uniqueness checks.
        pass
    return data
...
def route_target(site: Path, href: str) -> Path:
    relative = href[len(BASE_PATH) :].strip("/")
...
def render_style() -> str:
    css = """
:where(h1,h2,h3,[id]){scroll-margin-top:6.5rem}
:focus-visible{outline:3px solid #8b2f5b;outline-offset:3px;border-radius:.25rem}
.breadcrumbs{padding:.75rem 0;border-bottom:1px solid #c6e2df;background:#f7fcfb}
.breadcrumbs ol{display:flex;gap:.45rem;flex-wrap:wrap;list-style:none;padding:0;margin:0}
.breadcrumbs li:not(:last-child)::after{content:'←';margin-inline-start:.45rem;color:#607b7d}
.breadcrumbs a{text-decoration:none;font-weight:800}
.postlaunch{padding:1rem 0 2.5rem}
.postlaunch-grid{display:grid;grid-template-columns:1fr 1.25fr;gap:1rem;align-items:start}
...
.use-card ol,.policy-card ul{margin-block-end:0}
...
.related-card h3{font-size:1.05rem;line-height:1.5;margin:.1rem 0 .45rem}
.related-card p{color:#506d70;margin:.1rem 0 .85rem}
.related-card a{margin-top:auto;font-weight:900}
...
.policy-card{margin-top:1rem;border-inline-start:6px solid #8b2f5b}
@media(max-width:820px){.postlaunch-grid,.related-grid{grid-template-columns:1fr}}
...
def render_breadcrumbs(slug: str, label: str) -> str:
    body = (
        ...
        f'<li><a href="{BASE_PATH}">الرئيسية</a></li>'
        f'<li><a href="{BASE_PATH}special-needs/">ذوو الاحتياجات الخاصة</a></li>'
        f'<li><span aria-current="page">{esc(label)}</span></li>'
        '</ol></div></nav>'
    )
...
def guide_cards(links: list[dict[str, Any]]) -> str:
    ...
    for item in links:
        cards.append(
            ...
            f'<a href="{esc(item["href"])}">فتح الدليل العملي</a>'
            '</article>'
        )
    return "".join(cards)
...
def render_content(config: dict[str, Any], slug: str) -> str:
    ...
    body = f'''
<section class="postlaunch" id="use-this-guide" aria-labelledby="use-this-guide-title">
  <div class="wrap">
    <p class="audit-badge">تدقيق تقني وربط داخلي v{VERSION} · {esc(config["reviewed_at"])}</p>
    <div class="postlaunch-grid">
      <section class="use-card">
        <p class="kicker">طريقة استخدام عملية</p>
        <h2 id="use-this-guide-title">كيف تستخدم هذا الدليل دون تشخيص ذاتي؟</h2>
        <ol>{steps}</ol>
      </section>
      <section aria-labelledby="related-guides-title">
        <p class="kicker">مسارات مترابطة</p>
        <h2 id="related-guides-title">أدلة عملية مرتبطة بالحاجة اليومية</h2>
        <div class="related-grid">{guide_cards(links)}</div>
      </section>
    </div>
    <aside class="policy-card" id="provider-listing-policy" aria-labelledby="provider-policy-title">
      <p class="kicker">شفافية الدليل المحلي</p>
      <h2 id="provider-policy-title">{esc(policy["title"])}</h2>
      <ul>{policy_points}</ul>
    </aside>
  </div>
</section>
...
def validate_page(site: Path, slug: str, source: str, expected_links: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "slug": slug,
        "path": f"special-needs/{slug}/index.html",
        "words": visible_word_count(source),
        "h2": len(re.findall(r"<h2\b", source)),
        "html_ids": len(ids),
        "related_link_count": len(injected_hrefs),
        "meta_enhanced": True,
        "visible_breadcrumbs": True,
        "provider_policy_visible": True,
        "focus_visibility_guard": True,
    }
...
def publish(site: Path) -> dict[str, Any]:
    ...
    report = {
        ...
        "minimum_words": min(item["words"] for item in pages),
        "minimum_h2": min(item["h2"] for item in pages),
    ...
    (api / "special-needs-condition-postlaunch-v305.json").write_text(
