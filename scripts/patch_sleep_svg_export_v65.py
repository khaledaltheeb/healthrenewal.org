...
def patch() -> None:
    """Normalize the generated sleep chart and protect JS-managed controls.

    The chart uses one explicit accessible name on the SVG. Removing the nested
    SVG title/desc pair prevents generic HTML integrity audits from counting a
    second document title, while the preceding live text remains the detailed
    changing description. Export controls and privacy disclosure stay managed
    by sleep-log-v49.js so their tested bindings and IDs are not duplicated.
    """
    if not PAGE.is_file():
