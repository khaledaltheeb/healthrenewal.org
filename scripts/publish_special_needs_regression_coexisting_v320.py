...
CONTENT = ROOT / "content" / "v320" / "special-needs-regression-coexisting-ar.json"
BASE = "https://healthrenewal.org"
BP = "/"
VERSION = 320
MARKER = "data-regression-coexisting-v320"
...
def validate_payload(payload: dict) -> list[dict]:
    if payload.get("version") != VERSION or payload.get("language") != "ar":
        raise SystemExit("Regression/coexisting manifest contract failed")
    if payload.get("review_status") != "internally-reviewed-external-clinical-review-required":
        raise SystemExit("Regression/coexisting review status must remain honest")
    guides = payload.get("guides")
    if not isinstance(guides, list) or len(guides) != 2:
        raise SystemExit("Exactly two regression/coexisting guides are required")
    if {guide.get("slug") for guide in guides} != {
    ...
    if {guide.get("parent_slug") for guide in guides} != {"autism", "down-syndrome"}:
        raise SystemExit("Regression/coexisting parent routes are incomplete")
    ...
    for guide in guides:
        ...
        if not isinstance(sections, list) or len(sections) != 5:
            raise SystemExit(f"Each guide must contain five sections: {guide.get('slug')}")
        if not isinstance(sources, list) or len(sources) < 5:
            raise SystemExit(f"Each guide must contain at least five sources: {guide.get('slug')}")
        if len(guide.get("action_steps", [])) != 5 or len(guide.get("urgent", [])) != 3:
            raise SystemExit(f"Action or urgent contract failed: {guide.get('slug')}")

        source_index: dict[str, dict] = {}
        for source in sources:
            sid = str(source.get("id", "")).strip()
            url = str(source.get("url", "")).strip()
            if not sid or sid in all_ids or sid in source_index:
                raise SystemExit(f"Duplicate or empty source id: {sid}")
            if not is_https(url) or url in all_urls:
        ...
        for section in sections:
            ...
            if len(section.get("points", [])) != 6 or not refs or any(ref not in source_index for ref in refs):
                raise SystemExit(f"Invalid section evidence: {guide.get('slug')}/{section_id}")
            section_ids.add(section_id)
            used.update(refs)
        unused = sorted(set(source_index) - used)
        if unused:
            raise SystemExit(f"Unused regression/coexisting sources: {guide.get('slug')}/{unused}")

    if total_sources != 11:
        raise SystemExit(f"Regression/coexisting source total must be eleven, found {total_sources}")
    down = next(guide for guide in guides if guide["parent_slug"] == "down-syndrome")
    down_text = json.dumps(down, ensure_ascii=False)
    required_limits = (
        "إجماع خبراء",
    ...
    if missing_limits:
        raise SystemExit(f"DSRD/dementia limitation contract failed: {missing_limits}")
    return guides


CSS = """
*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;font-family:Tahoma,Arial,sans-serif;color:#173f43;background:#f5faf9;line-height:1.9}a{color:#066760}.wrap{width:min(1120px,92%);margin:auto}.skip{position:absolute;right:-9999px}.skip:focus{right:8px;top:8px;background:#fff;padding:9px;z-index:50}header{background:#123e43;color:#fff}.head{display:flex;justify-content:space-between;align-items:center;gap:14px;padding:12px 0}.head a{color:#fff;text-decoration:none;font-weight:800}nav{display:flex;gap:12px;flex-wrap:wrap}.hero{padding:54px 0 30px;background:linear-gradient(135deg,#e8f7f4,#fff)}h1{font-size:clamp(2rem,5vw,4rem);line-height:1.24;margin:.2em 0}h2{font-size:clamp(1.4rem,3vw,2rem);line-height:1.4}.lead{font-size:1.08rem;color:#48696c}.notice,.section-card,.panel,.sources{background:#fff;border:1px solid #c9e1de;border-radius:18px;padding:20px;box-shadow:0 12px 28px #123e4312}.notice{border-right:6px solid #8a3156}.grid{display:grid;grid-template-columns:260px 1fr;gap:20px;padding:30px 0}.toc{position:sticky;top:16px;align-self:start}.toc a{display:block;padding:6px 0;border-bottom:1px solid #e2efed;text-decoration:none}.stack{display:grid;gap:16px}.kicker{font-weight:900;color:#8a3156}.urgent{border-right:6px solid #a32626;background:#fff4f4}.actions{border-right:6px solid #08776f}.sources li{margin:1rem 0}.level{display:inline-block;background:#e7f8f4;border-radius:8px;padding:1px 7px;font-weight:900}.back{display:inline-block;background:#b9eee5;color:#123f43;text-decoration:none;font-weight:900;padding:9px 14px;border-radius:11px}footer{margin-top:30px;padding:26px 0;border-top:1px solid #c9e1de;color:#527174}@media(max-width:800px){.head,.grid{display:block}.head nav{margin-top:10px}.toc{position:static;margin-bottom:16px}}@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}}@media print{header,.skip,.toc{display:none}.grid{display:block}.section-card,.panel,.sources{box-shadow:none}}
"""
...
def render_guide(guide: dict, payload: dict) -> str:
    ...
    for section in guide["sections"]:
        ...
        sections.append(
            f'<section class="section-card" id="{e(section["id"])}"><p class="kicker">محور تقييم</p>'
    ...
    return f'''<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{e(guide['title'])}</title><meta name="description" content="{e(guide['meta_description'])}"><meta name="robots" content="index,follow,max-snippet:-1,max-image-preview:large"><link rel="canonical" href="{url}"><link rel="alternate" hreflang="ar" href="{url}"><link rel="alternate" hreflang="x-default" href="{url}"><meta property="og:type" content="article"><meta property="og:url" content="{url}"><meta property="og:title" content="{e(guide['title'])}"><meta property="og:description" content="{e(guide['meta_description'])}"><meta name="twitter:card" content="summary_large_image"><script type="application/ld+json">{schema(guide,payload)}</script><style>{CSS}</style></head><body><a class="skip" href="#main">انتقل إلى المحتوى</a><header><div class="wrap head"><a href="{BP}">منصة الصحة النفسية وذوي الاحتياجات الخاصة</a><nav><a href="{BP}">الرئيسية</a><a href="{BP}special-needs/">المركز</a><a href="{parent_path}">الدليل الشامل</a></nav></div></header><main id="main"><section class="hero"><div class="wrap"><p class="kicker">التغير الجديد يحتاج تفسيرًا لا افتراضًا</p><h1>{e(guide['short_title'])}</h1><p class="lead">{e(guide['lead'])}</p><p class="notice"><b>تنبيه:</b> {e(guide['warning'])}</p><p><a class="back" href="{parent_path}">العودة إلى الدليل العلمي الشامل</a></p></div></section><div class="wrap grid"><aside class="panel toc"><h2>محتويات الصفحة</h2>{toc}<a href="#actions">خطوات عملية</a><a href="#urgent">مؤشرات عاجلة</a><a href="#sources">المراجع</a></aside><article class="stack">{''.join(sections)}<section class="panel actions" id="actions"><h2>خطوات عملية للتقييم والمتابعة</h2><ol>{actions}</ol></section><section class="panel urgent" id="urgent"><h2>مؤشرات تستدعي تحركًا سريعًا</h2><ul>{urgent}</ul></section><section class="sources" id="sources"><h2>المراجع الأصلية</h2><ol>{sources}</ol><p><b>حالة المراجعة:</b> إعداد ومراجعة داخلية؛ لم تكتمل مراجعة سريرية خارجية مستقلة. آخر مراجعة {e(payload['reviewed_at'])}، والمراجعة التالية {e(payload['next_review_due'])}.</p></section></article></div></main><footer><div class="wrap"><p>محتوى تثقيفي لا يقدم تشخيصًا أو بروتوكول فحوص أو علاجًا فرديًا. استخدم خدمات الطوارئ المحلية عند الخطر.</p></div></footer></body></html>'''


def parent_block(guide: dict) -> str:
    return (
        f'<section class="section" {MARKER} data-regression-guide="{e(guide["slug"])}">'
        f'<div class="wrap"><p class="kicker">دليل التغير والتراجع</p><h2>{e(guide["short_title"])}</h2>'
        f'<p>{e(guide["meta_description"])}</p><a class="btn" href="{BP}special-needs/{e(guide["slug"])}/">فتح دليل التغير والتراجع</a></div></section>'
    )
...
def patch_parent(site: Path, guide: dict) -> None:
    ...
    pattern = rf'<section class="section" {MARKER} data-regression-guide="{re.escape(guide["slug"])}">.*?</section>'
    if re.search(pattern, text, flags=re.S):
        ...
        count = 1
    if count != 1 or text.count(f'data-regression-guide="{guide["slug"]}"') != 1:
        raise SystemExit(f"Parent regression-guide idempotence failed: {path}")
...
def update_sitemap(site: Path, guides: list[dict], updated: str) -> None:
    ...
    for guide in guides:
        ...
        for key, value in {"loc": url, "lastmod": updated, "changefreq": "monthly", "priority": "0.91"}.items():
            node = row.find(f"{{*}}{key}")
            if node is None:
...
def publish(site: Path) -> dict:
    ...
    for guide in guides:
        ...
        if page.count("<h1") != 1 or page.count('class="section-card"') != 5 or BANNED.search(page):
            raise SystemExit(f"Rendered regression/coexisting contract failed: {target}")
    ...
    report = {
        ...
        "dsrd_consensus_limit_visible": True,
        "dementia_baseline_limit_visible": True,
        "diagnostic_overshadowing_guard": True,
    ...
    (site / "api" / "special-needs-regression-coexisting-v320.json").write_text(
