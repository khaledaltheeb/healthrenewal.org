...
START = "<!-- special-needs-guides-v211:start -->"
END = "<!-- special-needs-guides-v211:end -->"
ALLOWED_HOSTS = {
    "www.who.int",
    "www.unicef.org",
    "www.cdc.gov",
...
def load_data() -> dict[str, Any]:
    ...
    for slug in manifest.get("guide_slugs", []):
        ...
        if not path.is_file():
            raise SystemExit(f"Missing v211 guide file: {path}")
...
def validate(data: dict[str, Any]) -> None:
    ...
    if missing:
        raise SystemExit(f"Missing v211 batch fields: {sorted(missing)}")
    if data["version"] != 211 or len(data["guides"]) != 5:
        raise SystemExit("v211 must contain exactly five guides")
    if data["status"] != "internally-reviewed":
        raise SystemExit("v211 must retain an honest internally-reviewed status")
    slugs = [guide["slug"] for guide in data["guides"]]
    if len(slugs) != len(set(slugs)):
        raise SystemExit("v211 guide slugs must be unique")
    ...
    if BANNED.search(serialized):
        raise SystemExit("Prohibited person-label language remains in v211 content")
...
def publish(site: Path) -> dict[str, Any]:
    ...
    report = {
        ...
        "guide_files": [f"content/v211/special-needs-guides/{guide['slug']}.json" for guide in data["guides"]],
    ...
    (api / "special-needs-guides-v211.json").write_text(
