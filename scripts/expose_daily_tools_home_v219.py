...
def patch_jsonld(text: str) -> str:
    ...
    collection = next(
        (
            ...
            and item.get("@type") == "CollectionPage"
            and str(item.get("@id", "")).endswith("#home")
        ),
        None,
    )
    if not collection:
    ...
    if not isinstance(parts, list):
        raise SystemExit("Homepage hasPart must be a list")
    additions = (
        {
            "@type": "CollectionPage",
        ...
        {
            "@type": "CollectionPage",
            "name": "مسارات تعلم الصحة النفسية",
            "url": BASE + "learning-paths/",
        },
    )
...
def patch_index() -> None:
    ...
    if "data-learning-paths-v219" not in text:
        cards += (
    ...
    if cards:
        if text.count(card_marker) != 1:
            raise SystemExit("Homepage cognitive-tools card marker changed")
