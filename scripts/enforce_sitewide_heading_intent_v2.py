...
The existing SEO audit already validates exactly one H1 and at least one H2.
This deterministic post-processor closes the remaining contract gap: every
indexable sitemap HTML page must also contain a meaningful H3 and enough visible
questions to reflect likely search intent. It only adds a fallback section when
the page does not already satisfy the stronger contract.
"""
...
def render_section(kind: str, label: str, description: str) -> str:
    items = intent_items(kind, label, description)
    parts = [
        f'<section class="seo-intent-answers" {MARKER} aria-labelledby="sitewide-heading-intent-v2-title">',
        '<h2 id="sitewide-heading-intent-v2-title">أسئلة تساعدك على فهم الصفحة واستخدامها</h2>',
    ]
