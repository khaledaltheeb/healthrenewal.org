...
START = "<!-- special-needs-guides-v212:start -->"
END = "<!-- special-needs-guides-v212:end -->"
ALLOWED_HOSTS = {
    "www.who.int",
    "www.unicef.org",
    "www.cdc.gov",
...
def load_data() -> dict[str, Any]:
    ...
    for slug in manifest.get("guide_slugs", []):
        ...
        if not path.is_file():
            raise SystemExit(f"Missing v212 guide file: {path}")
...
def validate(data: dict[str, Any]) -> None:
    ...
    if missing:
        raise SystemExit(f"Missing v212 batch fields: {sorted(missing)}")
    if data["version"] != 212 or len(data["guides"]) != 5:
        raise SystemExit("v212 must contain exactly five guides")
    if data["status"] != "internally-reviewed":
        raise SystemExit("v212 must retain an honest internally-reviewed status")
    slugs = [guide["slug"] for guide in data["guides"]]
    if len(slugs) != len(set(slugs)):
        raise SystemExit("v212 guide slugs must be unique")
    ...
    if BANNED.search(serialized):
        raise SystemExit("Prohibited person-label language remains in v212 content")
...
def publish(site: Path) -> dict[str, Any]:
    ...
    report = {
        ...
        "guide_files": [f"content/v212/special-needs-guides/{guide['slug']}.json" for guide in data["guides"]],
    ...
    (api / "special-needs-guides-v212.json").write_text(
