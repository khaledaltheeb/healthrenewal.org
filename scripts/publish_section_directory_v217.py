...
DISCOVERY_KEYWORDS = (
    "الصحة النفسية", "علم النفس", "مصطلحات علم النفس", "الموسوعة النفسية",
    "المكتبة النفسية", "المكتبة الأكاديمية", "مقارنات نفسية",
    "الفرق بين الاضطرابات النفسية", "الأشخاص ذوو الاحتياجات الخاصة",
    "ذوو الاحتياجات الخاصة", "التربية الدامجة", "التربية الخاصة",
    "الصحة النفسية للطفل", "الصحة النفسية للأسرة", "التقييم النفسي",
    "المقاييس النفسية", "الاختبارات النفسية", "القدرات المعرفية",
    "العلاج النفسي", "الدعم الأسري", "التدخل المبكر", "التوحد",
    "صعوبات التعلم", "جودة خدمات التربية الخاصة",
)
...
def replace_meta(source: str, attribute: str, key: str, content: str) -> str:
    pattern = rf'<meta\b[^>]*{attribute}=(["\']){re.escape(key)}\1[^>]*>'
    replacement = f'<meta {attribute}="{key}" content="{html.escape(content, quote=True)}">'
...
def strengthen_homepage_discovery(source: str) -> str:
    source = re.sub(
        r"<title>.*?</title>",
        f"<title>{BRAND} | موسوعة ومكتبة ومقارنات عربية</title>",
        source,
        count=1,
        flags=re.I | re.S,
    )
    source = replace_meta(
        ...
        "name",
        "description",
        "منصة عربية مؤسسية للصحة النفسية والأشخاص ذوي الاحتياجات الخاصة، تضم موسوعة ومكتبة أكاديمية ومقارنات وأدلة عملية ومقاييس استكشافية ومحتوى موثقًا يخدم الأسرة والمختص والمعلم.",
    )
    source = replace_meta(
        source,
        "property",
        "og:title",
        "منصة الصحة النفسية | موسوعة ومكتبة ومقارنات عربية",
    )
    source = replace_meta(
        source,
        "property",
        "og:description",
        "موسوعة ومكتبة أكاديمية ومقارنات نفسية وأدلة تطبيقية للأسرة والطفل والتربية الدامجة ومنصة تقييم استكشافية ضمن منهج موثق يحترم الإنسان.",
    )
    source = replace_meta(
        source,
        "name",
    ...
    source = replace_meta(
        ...
        "name",
        "twitter:description",
        "موسوعة ومكتبة ومقارنات وأدلة عربية موثوقة في الصحة النفسية والتربية الدامجة ودعم الأسرة والمختص.",
    )
    ...
    existing = []
    if keyword_match:
        existing = [item.strip() for item in keyword_match.group(3).split(",") if item.strip()]
    merged: list[str] = []
    for item in [*existing, *DISCOVERY_KEYWORDS]:
        if item not in merged:
            merged.append(item)
    source = replace_meta(source, "name", "keywords", ",".join(merged[:30]))

    discovery_links = (
        f'<link rel="sitemap" type="application/xml" href="{BASE_URL}sitemap.xml">',
...
def inject_direct_navigation(source: str) -> str:
    match = re.search(r'(<nav\b[^>]*class=(["\'])nav\2[^>]*>)(.*?)(</nav>)', source, re.I | re.S)
    if not match:
        raise SystemExit("Primary navigation could not be found")
    body = match.group(3)
    for route, label in DIRECT_NAVIGATION:
        if not re.search(rf'href=(["\']){re.escape(route)}\1', body, re.I):
            body += f'<a href="{route}">{label}</a>'
    return source[:match.start()] + match.group(1) + body + match.group(4) + source[match.end():]
...
def inject_homepage(records: list[dict[str, object]]) -> tuple[str, int]:
    ...
    source = strengthen_homepage_discovery(source)
    source = inject_direct_navigation(source)
    style = (
...
def main() -> None:
    ...
    direct_navigation = {
        route: bool(re.search(rf'<nav\b[^>]*class=(["\'])nav\1[^>]*>.*?href=(["\']){re.escape(route)}\2', homepage_after, re.I | re.S))
        for route, _label in DIRECT_NAVIGATION
    }
    if not all(featured_status.values()):
        raise SystemExit(f"Featured sections are not directly linked: {featured_status}")
    if not all(direct_navigation.values()):
        raise SystemExit(f"Primary navigation is incomplete: {direct_navigation}")
    keywords = re.search(r'<meta\b[^>]*name=(["\'])keywords\1[^>]*content=(["\'])(.*?)\2', homepage_after, re.I | re.S)
    keyword_count = len([item for item in keywords.group(3).split(",") if item.strip()]) if keywords else 0
    if keyword_count < 20:
        raise SystemExit(f"Homepage keyword coverage is too narrow: {keyword_count}")
    report = {
        "version": 217,
        "status": "passed",
        "section_count": len(records),
        ...
        "comparisons_linked": featured_status["comparisons/"],
        "library_linked": featured_status["library/"],
        "direct_navigation": direct_navigation,
        "homepage_keyword_count": keyword_count,
