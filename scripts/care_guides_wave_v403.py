...
import care_guides_wave_v402 as previous_wave
from care_guides_topics_v403_1 import TOPICS as TOPICS_1
from care_guides_topics_v403_2 import TOPICS as TOPICS_2
from care_guides_topics_v403_3 import TOPICS as TOPICS_3
from care_guides_topics_v403_4 import TOPICS as TOPICS_4
from care_guides_topics_v403_5 import TOPICS as TOPICS_5

VERSION = 403
RELEASE_DATE = "2026-08-14"
WAVE = 4
EXPECTED_WAVE_GUIDES = 50
EXPECTED_INSTITUTIONAL_GUIDES = 287
EXPECTED_SOURCE_GUIDES = 301
MINIMUM_PUBLISHED_GUIDES = 300
...
def topics() -> list[tuple[str, ...]]:
    ...
    if len(slugs) != len(set(slugs)) or len(titles) != len(set(titles)):
        raise RuntimeError("Wave 004 contains duplicate slugs or titles")
    return values


def install(implementation: object) -> dict[str, object]:
    if getattr(implementation, "_care_guides_wave_v403_installed", False):
        return getattr(implementation, "_care_guides_wave_v403_report")
    ...
    if collisions or title_collisions:
        raise RuntimeError(f"Wave 004 collides with existing guides: {collisions} {title_collisions}")
    ...
    if len(wave_guides) != EXPECTED_WAVE_GUIDES:
        raise RuntimeError("Wave 004 did not generate exactly 50 guides")
    ...
    implementation._care_guides_wave_v403_installed = True
    implementation._care_guides_wave_v403_report = report
    return report
