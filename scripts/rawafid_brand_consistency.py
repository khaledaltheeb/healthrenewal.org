...
TAGLINE_AR = "للعافية النفسية والدمج والتمكين"
DESCRIPTION_AR = (
...
SELF_NAMES = {
    "rawafid_brand_consistency.py",
    "sitecustomize.py",
    "apply_rawafid_brand.py",
}
...
LEGACY_LITERAL_EN = ("Psychology Terminology",)
...
class Report:
    ...
    legacy_examples: list[str] | None = None
    changed_examples: list[str] | None = None
...
def build_meta(key: str, content: str, lang: str) -> str:
    attr = "property" if key in OG_KEYS else "name"
    safe = content.replace('"', "&quot;")
    return f'<meta {attr}="{key}" content="{safe}">'


def meta_key(tag: str) -> tuple[str | None, str | None]:
    values = attrs(tag)
    if "name" in values:
        return "name", values["name"].strip().lower()
    if "property" in values:
        return "property", values["property"].strip().lower()
    return None, None


def first_meta_content(text: str, key: str) -> str | None:
    for match in META_TAG_RE.finditer(text):
        _, found = meta_key(match.group(0))
        if found == key:
...
def normalize_meta(text: str, path: Path) -> tuple[str, int]:
    ...
    def replace_meta(match: re.Match[str]) -> str:
        ...
        if key in seen:
            ...
            return ""
        seen.add(key)
        content = fixed.get(key)
        if content is None:
            content = attrs(tag).get("content", "").strip() or defaults.get(key, "")
        return build_meta(key, content, lang)

    text = META_TAG_RE.sub(replace_meta, text)

    additions: list[str] = []
    for key, value in {**fixed, **defaults}.items():
        if key not in seen:
            additions.append(build_meta(key, value, lang))
            seen.add(key)
...
def normalize_brand_assets(text: str) -> str:
    def is_brand_link(tag: str) -> bool:
        values = attrs(tag)
        rel = values.get("rel", "").lower()
        href = values.get("href", "")
        if "icon" in rel or rel == "manifest":
            return True
        if "rawafid-brand.css" in href:
            return True
        return False

    text = LINK_TAG_RE.sub(lambda match: "" if is_brand_link(match.group(0)) else match.group(0), text)

    def is_brand_script(tag: str) -> bool:
        return "rawafid-brand.js" in attrs(tag).get("src", "")

    text = SCRIPT_TAG_RE.sub(lambda match: "" if is_brand_script(match.group(0)) else match.group(0), text)
...
def normalize_hreflang(text: str, root: Path, production: bool) -> tuple[str, int]:
    ...
    def replace_link(match: re.Match[str]) -> str:
        ...
        if "alternate" not in rel or "hreflang" not in values or "href" not in values:
            return tag
        target = local_target(root, values["href"])
        if target is not None and not target.is_file():
            removed += 1
            return ""
        return tag

    return LINK_TAG_RE.sub(replace_link, text), removed
...
def normalize_html(text: str, path: Path, root: Path, production: bool) -> tuple[str, dict[str, int]]:
    ...
    text, count = replace_legacy_text(text, path)
    stats["replacements"] += count
    if not HEAD_CLOSE_RE.search(text):
        return text, stats
    text, duplicates = normalize_meta(text, path)
    text = normalize_brand_assets(text)
    stats["duplicates"] += duplicates
    text, removed = normalize_hreflang(text, root, production)
    stats["hreflang_removed"] += removed
    return text, stats
...
def normalize_manifest(root: Path, fix: bool) -> tuple[bool, list[str], bool]:
    ...
    if icons != updated["icons"]:
        errors.append("icons do not match the canonical PWA contract")
    return not errors, errors, changed


def count_duplicate_meta(text: str) -> int:
    counts: dict[str, int] = {}
    for match in META_TAG_RE.finditer(text):
        _, key = meta_key(match.group(0))
...
def legacy_count(text: str) -> int:
    count = sum(text.count(value) for value in LEGACY_LITERAL_AR + LEGACY_LITERAL_EN)
    count += len(LEGACY_HEALTH_RENEWAL_RE.findall(text))
    return count


def audit(root: Path, production: bool, base: Report) -> Report:
    legacy_examples: list[str] = []
    changed_examples = base.changed_examples or []
    assets = (
    ...
    base.missing_assets = [name for name in assets if not (root / name).is_file() or (root / name).stat().st_size == 0]
    ...
    for path in iter_text_files(root, production):
        try:
            ...
            continue
        base.text_files_scanned += 1
        occurrences = legacy_count(text)
        if occurrences:
            base.legacy_occurrences += occurrences
            base.legacy_files += 1
            if len(legacy_examples) < 10:
                legacy_examples.append(str(path.relative_to(root)))
        if path.suffix.lower() not in {".html", ".htm"}:
            continue
        base.html_files += 1
        if not HEAD_CLOSE_RE.search(text):
            base.html_missing_head += 1
            continue
        if "/assets/brand/rawafid-brand.css" not in text:
            base.html_missing_brand_style += 1
        if "/assets/brand/rawafid-brand.js" not in text:
            base.html_missing_brand_script += 1
        if first_meta_content(text, "application-name") is None:
            base.html_missing_application_name += 1
        if first_meta_content(text, "og:site_name") is None:
            base.html_missing_og_site_name += 1
        if first_meta_content(text, "theme-color") is None:
            base.html_missing_theme_color += 1
        if "/manifest.webmanifest" not in text:
            base.html_missing_manifest_link += 1
        base.html_duplicate_meta_tags += count_duplicate_meta(text)

        normalized, _ = normalize_html(text, path, root, production)
        if normalized != text:
            base.files_needing_changes += 1

    valid_manifest, manifest_errors, manifest_would_change = normalize_manifest(root, fix=False)
    base.manifest_valid = valid_manifest
    base.manifest_errors = manifest_errors
    if manifest_would_change:
        base.files_needing_changes += 1
    base.legacy_examples = legacy_examples
    base.changed_examples = changed_examples[:20]

    blocking = (
        base.legacy_occurrences
        + base.html_missing_head
        + base.html_missing_brand_style
        + base.html_missing_brand_script
        + base.html_missing_application_name
        + base.html_missing_og_site_name
        + base.html_missing_theme_color
        + base.html_missing_manifest_link
        + base.html_duplicate_meta_tags
        + base.files_needing_changes
        + len(base.missing_assets)
        + len(base.manifest_errors or [])
    )
    base.status = "passed" if blocking == 0 and base.html_files > 0 else "failed"
    return base
...
def normalize_root(
    ...
    *,
    ...
    root = root.resolve()
    report = Report(root=str(root), production_mode=production, changed_examples=[])
    for path in iter_text_files(root, production):
        try:
            text = path.read_text(encoding="utf-8")
        except UnicodeDecodeError:
        ...
        if updated != text:
            ...
            if len(report.changed_examples or []) < 20:
                assert report.changed_examples is not None
    ...
    if manifest_changed:
        ...
        if len(report.changed_examples or []) < 20:
            assert report.changed_examples is not None
            report.changed_examples.append("manifest.webmanifest")

    final = Report(
        root=str(root),
        production_mode=production,
        files_changed=report.files_changed,
        text_replacements=report.text_replacements,
        duplicate_meta_tags_removed=report.duplicate_meta_tags_removed,
    ...
    if report_path is None:
        report_path = (
            root / "api/rawafid-brand-consistency-v2.json"
            if production
            else root / "reports/rawafid-brand-consistency-v2.json"
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Normalize and audit Rawafid branding.")
    parser.add_argument("--root", default=".", help="Repository root or generated _site directory.")
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument("--fix", action="store_true", help="Apply safe deterministic fixes before auditing.")
    mode.add_argument("--check", action="store_true", help="Audit only; exit non-zero if any inconsistency exists.")
    parser.add_argument("--production", action="store_true", help="Enable generated-site checks such as hreflang target validation.")
...
def main() -> int:
    ...
    result = normalize_root(root, fix=bool(args.fix), production=bool(args.production), report_path=report_path)
