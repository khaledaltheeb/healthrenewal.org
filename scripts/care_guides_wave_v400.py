...
from care_guides_topics_v400_1 import TOPICS as TOPICS_1
from care_guides_topics_v400_2 import TOPICS as TOPICS_2
from care_guides_topics_v400_3a import TOPICS as TOPICS_3A
from care_guides_topics_v400_3b import TOPICS as TOPICS_3B

VERSION = 400
RELEASE_DATE = "2026-08-13"
WAVE = 1
EXPECTED_WAVE_GUIDES = 50
EXPECTED_INSTITUTIONAL_GUIDES = 137
EXPECTED_SOURCE_GUIDES = 151
MINIMUM_PUBLISHED_GUIDES = 150
TARGET_TOTAL_PAGES = 10000


def topics() -> list[tuple[str, ...]]:
    values = [*TOPICS_1, *TOPICS_2, *TOPICS_3A, *TOPICS_3B]
    ...
    if len(slugs) != len(set(slugs)) or len(titles) != len(set(titles)):
        raise RuntimeError("Wave 001 contains duplicate slugs or titles")
    return values


def install(implementation: object) -> dict[str, object]:
    if getattr(implementation, "_care_guides_wave_v400_installed", False):
        return getattr(implementation, "_care_guides_wave_v400_report")
    ...
    if collisions or title_collisions:
        raise RuntimeError(f"Wave 001 collides with existing guides: {collisions} {title_collisions}")

    # institutional_guides() reads TOPICS_4 and EXPECTED_GUIDES from the catalog
    # module at call time, so extending the catalog here preserves the established
    # v246 builder, source bundles, safety language and publication pipeline.
    ...
    if len(wave_guides) != EXPECTED_WAVE_GUIDES:
        raise RuntimeError("Wave 001 did not generate exactly 50 guides")

    category_distribution = Counter(guide["category"] for guide in wave_guides)
    source_minimum = min(len(guide.get("sources", [])) for guide in wave_guides)
    report = {
        "version": VERSION,
        "wave": WAVE,
        ...
        "minimum_sources": source_minimum,
        "category_distribution": dict(sorted(category_distribution.items())),
        "specialist_review_claimed": False,
    }
    implementation._care_guides_wave_v400_installed = True
    implementation._care_guides_wave_v400_report = report
    return report
