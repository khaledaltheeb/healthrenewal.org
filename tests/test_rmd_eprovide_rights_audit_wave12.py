...
LEDGER=ROOT/'content'/'global-measures-v1'/'rmd-eprovide-rights-audit.json'
CATALOG=ROOT/'content'/'global-measures-v1'/'catalog.json'
PAGE=ROOT/'assessments'/'global-measures'/'rights-audit'/'index.html'
JS=PAGE.with_name('audit.js')
...
class RmdEprovideRightsAuditWave12Test(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  cls.ledger=json.loads(LEDGER.read_text(encoding='utf-8'));cls.catalog=json.loads(CATALOG.read_text(encoding='utf-8'));cls.page=PAGE.read_text(encoding='utf-8');cls.js=JS.read_text(encoding='utf-8');cls.xml=SITEMAP.read_text(encoding='utf-8');cls.phq=PHQ.read_text(encoding='utf-8');cls.gad=GAD.read_text(encoding='utf-8')
 ...
 def test_owner_controlled_priority_tools_cannot_be_full(self):
  ...
  for key in ('odi','ndi','quickdash','eq5d5l','isi','zbi'):
   self.assertEqual(rows[key]['decision'],'official-link-only');self.assertNotIn(rows[key]['decision'],('full-ok','full-ok-with-attribution'))
 def test_current_catalog_is_reconciled_to_29(self):
  self.assertEqual(self.catalog['actual_tool_count'],29);self.assertEqual(len(self.catalog['tools']),29);self.assertEqual(len({x['id'] for x in self.catalog['tools']}),29);self.assertIn('healthy-days',{x['id'] for x in self.catalog['tools']})
 def test_audit_page_uses_single_ledger(self):
  self.assertIn('/content/global-measures-v1/rmd-eprovide-rights-audit.json',self.js);self.assertIn('RMD Cost ≠ reproduction rights',self.page);self.assertIn('RMDQ‑24 حالة خاصة',self.page);self.assertIn('OPERATIONAL SHEET',self.page);self.assertNotIn('localStorage',self.js)
