...
PAGE=ROOT/'sectors'/'rehabilitation'/'measures'/'nine-hole-peg-test'/'index.html';JS=PAGE.with_name('9hpt.js');REG=ROOT/'content'/'global-measures-v1'/'nine-hole-peg-test.json';CAT=ROOT/'content'/'global-measures-v1'/'catalog.json';SITEMAP=ROOT/'sitemap-rehabilitation-measures.xml';PACKET=ROOT/'assessments'/'global-measures'/'print-packets'/'upper-limb-dexterity'/'index.html';NS={'s':'http://www.sitemaps.org/schemas/sitemap/0.9'}
class NineHolePegWave19Test(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  cls.html=PAGE.read_text(encoding='utf-8');cls.js=JS.read_text(encoding='utf-8');cls.reg=json.loads(REG.read_text(encoding='utf-8'));cls.catalog=json.loads(CAT.read_text(encoding='utf-8'));cls.xml=SITEMAP.read_text(encoding='utf-8');cls.packet=PACKET.read_text(encoding='utf-8')
