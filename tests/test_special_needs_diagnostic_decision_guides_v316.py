...
class SpecialNeedsDiagnosticDecisionGuidesV316Tests(unittest.TestCase):
    ...
    def test_publish_generates_two_decision_guides_with_honest_boundaries(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            site = self.make_site(Path(tmp))
            report = decision316.publish(site)
            ...
            self.assertEqual(report["decision_step_count"], 10)
            ...
            expected = {
                "autism-screening-vs-diagnosis": {
                    "parent": "autism",
                    "source_ids": ("AS1", "AS2", "AS3", "AS4"),
                    "required": (
                        "أداة التحري تجيب عن سؤال",
                        "النتيجة الإيجابية تعني زيادة الحاجة إلى تقييم أعمق",
                        "لا يعتمد على أداة تشخيصية واحدة وحدها",
                    ),
                },
                "down-syndrome-prenatal-screening-vs-diagnosis": {
                    "parent": "down-syndrome",
                    "source_ids": ("DP1", "DP2", "DP3", "DP4"),
                    "required": (
                        "للمريضة الحق في قبول التحري أو الاختبار التشخيصي أو رفضهما",
                        "cfDNA هو أكثر اختبارات التحري حساسية ونوعية",
                        "لا تفسر نتيجة تحرٍ إيجابية على أنها تشخيص مؤكد",
                        "لا يتنبأ بدقة كاملة",
                    ),
                },
            }
            for slug, spec in expected.items():
                ...
                self.assertIsNone(decision316.BANNED.search(page))
            ...
            self.assertEqual(api["content_source"], "content/v316/special-needs-diagnostic-decision-guides-ar.json")
            ...
            for slug in expected:
                self.assertEqual(locations.count(f"{decision316.BASE}/special-needs/{slug}/"), 1)
            self.assertEqual(len(locations), len(set(locations)))

    def test_publication_is_byte_stable(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            site = self.make_site(Path(tmp))
            decision316.publish(site)
            tracked = [
                site / "special-needs" / "autism" / "index.html",
                site / "special-needs" / "down-syndrome" / "index.html",
                site / "special-needs" / "autism-screening-vs-diagnosis" / "index.html",
                site / "special-needs" / "down-syndrome-prenatal-screening-vs-diagnosis" / "index.html",
                site / "sitemap-special-needs.xml",
                site / "api" / "special-needs-diagnostic-decision-guides-v316.json",
            ]
            before = [self.digest(path) for path in tracked]
            second = decision316.publish(site)
    ...
    def test_manifest_rejects_unused_sources(self) -> None:
        payload = decision316.read_json(decision316.CONTENT)
        payload["guides"][0]["sources"].append(
            {
                "id": "ASX",
                "organization": "Example",
                "title": "Unused",
                "url": "https://example.org/unused-v316",
                "level": "S5",
                "reviewed": "2026-01-01",
            }
        )
        with self.assertRaises(SystemExit):
            decision316.validate_payload(payload)
