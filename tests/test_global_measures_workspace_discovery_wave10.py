...
class WorkspaceDiscoveryWave10Test(unittest.TestCase):
 ...
 def test_global_gateway_preserves_wave10_minimum_and_workspace_links(self):
  ...
  for route in ('/assessments/global-measures/workspace/','/assessments/global-measures/tool-finder/','/assessments/global-measures/print-packets/','/assessments/global-measures/measurement-record/','/assessments/global-measures/rights-audit/'): self.assertIn(route,self.global_html)
 def test_workspace_wires_runtime_and_rights_components(self):
  for route in ('/assessments/global-measures/tool-finder/','/assessments/global-measures/print-packets/','/assessments/global-measures/measurement-record/','/assessments/global-measures/rights-audit/','/sectors/rehabilitation/measures/arabic-readiness/'): self.assertIn(route,self.work)
  self.assertIn('RMD Cost',self.work);self.assertIn('ePROVIDE',self.work)
 def test_sitemap_discovers_workspace_pages_once(self):
  urls=[el.text for el in ET.fromstring(self.xml).findall('s:url/s:loc',NS)]
  for url in ('https://healthrenewal.org/assessments/global-measures/workspace/','https://healthrenewal.org/assessments/global-measures/tool-finder/','https://healthrenewal.org/assessments/global-measures/print-packets/','https://healthrenewal.org/assessments/global-measures/measurement-record/','https://healthrenewal.org/assessments/global-measures/rights-audit/'): self.assertEqual(urls.count(url),1)
