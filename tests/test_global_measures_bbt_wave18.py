...
PAGE=ROOT/'sectors'/'rehabilitation'/'measures'/'box-and-block-test'/'index.html';JS=PAGE.with_name('bbt.js');REG=ROOT/'content'/'global-measures-v1'/'box-and-block-test.json';CAT=ROOT/'content'/'global-measures-v1'/'catalog.json';AUDIT=ROOT/'content'/'global-measures-v1'/'rmd-eprovide-rights-audit.json';LEGACY_OVERRIDE=ROOT/'content'/'rehabilitation-measures-v1'/'registry-overrides.json';LIB=ROOT/'sectors'/'rehabilitation'/'measures'/'app.js';SITEMAP=ROOT/'sitemap-rehabilitation-measures.xml';PACKETS=ROOT/'assessments'/'global-measures'/'print-packets'/'index.html';NS={'s':'http://www.sitemaps.org/schemas/sitemap/0.9'}
class BoxAndBlockWave18Test(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  cls.html=PAGE.read_text(encoding='utf-8');cls.js=JS.read_text(encoding='utf-8');cls.reg=json.loads(REG.read_text(encoding='utf-8'));cls.catalog=json.loads(CAT.read_text(encoding='utf-8'));cls.audit=json.loads(AUDIT.read_text(encoding='utf-8'));cls.override=json.loads(LEGACY_OVERRIDE.read_text(encoding='utf-8'));cls.lib=LIB.read_text(encoding='utf-8');cls.xml=SITEMAP.read_text(encoding='utf-8');cls.packets=PACKETS.read_text(encoding='utf-8')
 ...
 def test_integration_as_tool_35(self):
  self.assertEqual(self.catalog['actual_tool_count'],35);self.assertEqual(len(self.catalog['tools']),35);row=next(x for x in self.catalog['tools'] if x['id']=='bbt');self.assertEqual(row['route'],'/sectors/rehabilitation/measures/box-and-block-test/');self.assertEqual(row['rights_state'],'rmd-not-free-equipment-operational-sheet-rights-not-public-domain-verified');rights=next(x for x in self.audit['current_library'] if x['id']=='bbt');self.assertEqual(rights['decision'],'operational-sheet-only');self.assertEqual(rights['rmd_cost'],'Not Free');self.assertIn("bbt='/sectors/rehabilitation/measures/box-and-block-test/'",self.lib);self.assertIn('حزمة الطرف العلوي والبراعة اليدوية',self.packets);urls=[e.text for e in ET.fromstring(self.xml).findall('s:url/s:loc',NS)];self.assertEqual(urls.count('https://healthrenewal.org/sectors/rehabilitation/measures/box-and-block-test/'),1)
 def test_legacy_registry_is_reconciled_by_explicit_override_and_runtime(self):
  self.assertIn('precedence_rule',self.override);ov=next(x for x in self.override['overrides'] if x['id']=='box-block');self.assertEqual(ov['legacy_status'],'verify-before-reproduction');self.assertEqual(ov['current_status'],'operational-sheet-only');self.assertEqual(ov['route'],'/sectors/rehabilitation/measures/box-and-block-test/');self.assertIn("match:'Box and Block Test'",self.lib);self.assertIn("label:'ورقة تشغيل متاحة'",self.lib);self.assertIn("route:'/sectors/rehabilitation/measures/box-and-block-test/'",self.lib)
