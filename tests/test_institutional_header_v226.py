...
from scripts.publish_institutional_header_v226 import (
    LANGUAGE_LINKS,
    SECTION_LINKS,
    publish,
    validate,
...
class InstitutionalHeaderV226Tests(unittest.TestCase):
    def test_publisher_replaces_legacy_header_and_is_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(first_text.count("data-institutional-header-v226"), 1)
            self.assertEqual(first_text.count('id="institutional-header-v226-styles"'), 1)
            ...
            report = json.loads(
                (site / "api" / "institutional-header-v226.json").read_text(
                    encoding="utf-8"
                )
            )
    ...
    def test_all_requested_sections_and_languages_are_present(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual({label for label, _ in SECTION_LINKS}, requested_sections)
            self.assertEqual(
