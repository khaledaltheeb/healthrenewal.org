...
class GlobalMeasuresGatewayTest(unittest.TestCase):
 ...
 def test_gateway_count_matches_machine_catalog(self):
  m=re.search(r'<strong>(\d+) أداة/بطارية فعلية حاليًا</strong>',self.html);self.assertIsNotNone(m);self.assertEqual(int(m.group(1)),self.catalog['actual_tool_count']);self.assertEqual(self.catalog['actual_tool_count'],32);self.assertEqual(len(self.catalog['tools']),32);self.assertIn('Berg Balance Scale',self.html);self.assertIn('Modified Ashworth Scale',self.html);self.assertIn('Modified Rankin Scale',self.html);self.assertIn('20 أداة/بطارية',self.html)
 def test_rmd_rights_model_is_explicit(self):
  ...
  bbs=next(x for x in self.audit['current_library'] if x['id']=='bbs');self.assertEqual(bbs['decision'],'full-ok');mas=next(x for x in self.audit['current_library'] if x['id']=='mas');self.assertEqual(mas['decision'],'full-ok');mrs=next(x for x in self.audit['current_library'] if x['id']=='mrs');self.assertEqual(mrs['decision'],'full-ok');self.assertEqual(mrs['rmd_cost'],'Free');self.assertIn('Public Domain',mrs['rights_source'])
