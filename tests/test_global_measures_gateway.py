...
class GlobalMeasuresGatewayTest(unittest.TestCase):
 ...
 def test_gateway_count_matches_machine_catalog(self):
  m=re.search(r'<strong>(\d+) أداة/بطارية فعلية حاليًا</strong>',self.html);self.assertIsNotNone(m);self.assertEqual(int(m.group(1)),self.catalog['actual_tool_count']);self.assertEqual(self.catalog['actual_tool_count'],35);self.assertEqual(len(self.catalog['tools']),35);self.assertIn('Box and Block Test',self.html);self.assertIn('Functional Ambulation Category',self.html);self.assertIn('Modified Tardieu Scale',self.html);self.assertIn('Modified Rankin Scale',self.html);self.assertIn('23 أداة/بطارية',self.html)
