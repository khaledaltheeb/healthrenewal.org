...
class GlobalMeasuresGatewayTest(unittest.TestCase):
 ...
 def test_gateway_count_matches_machine_catalog(self):
  m=re.search(r'<strong>(\d+) أداة/بطارية فعلية حاليًا</strong>',self.html);self.assertIsNotNone(m);self.assertEqual(int(m.group(1)),self.catalog['actual_tool_count']);self.assertEqual(self.catalog['actual_tool_count'],39);self.assertEqual(len(self.catalog['tools']),39);self.assertIn('Action Research Arm Test',self.html);self.assertIn('Timed 25‑Foot Walk',self.html);self.assertIn('Goal Attainment Scaling',self.html);self.assertIn('27 أداة/بطارية',self.html)
 def test_rmd_rights_model_and_operational_methods_are_explicit(self):
  self.assertIn('RMD لا يملك الأدوات',self.html);self.assertIn('Cost + Cost Description',self.html);self.assertIn('operational-sheet-only',self.html);rows={x['id']:x for x in self.audit['current_library']};self.assertEqual(rows['t25fw']['decision'],'operational-sheet-only');self.assertEqual(rows['t25fw']['rmd_cost'],'Free');self.assertIn('MSFC',rows['t25fw']['copyright_state']);self.assertEqual(rows['arat']['decision'],'operational-sheet-only');self.assertEqual(rows['arat']['rmd_cost'],'Not Free');self.assertIn('equipment cost',rows['arat']['rmd_cost_description'].lower());self.assertEqual(rows['gas']['decision'],'operational-sheet-only')
 def test_lifecycle_ids_do_not_overlap(self):self.assertTrue({x['id'] for x in self.audit['current_library']}.isdisjoint({x['id'] for x in self.audit['priority_candidates']}))
