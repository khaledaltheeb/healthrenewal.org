...
ROUTES=(
 ...
 '/assessments/distress-screeners/',
 '/assessments/geriatric-measures/',
 '/assessments/substance-use-screeners/',
 '/assessments/general-health-measures/',
 '/assessments/global-measures/workspace/',
...
class GlobalMeasuresGatewayTest(unittest.TestCase):
 @classmethod
 def setUpClass(cls):
  cls.html=PAGE.read_text(encoding='utf-8');cls.sitemap=SITEMAP.read_text(encoding='utf-8');cls.catalog=json.loads(CATALOG.read_text(encoding='utf-8'));cls.audit=json.loads(AUDIT.read_text(encoding='utf-8'))
 def test_identity_and_metadata(self):
  self.assertIn('المقاييس وأدوات التقييم المستخدمة عالميًا',self.html);self.assertIn('rel="canonical" href="https://healthrenewal.org/assessments/global-measures/"',self.html);self.assertIn('name="robots" content="index,follow',self.html);self.assertEqual(self.html.count('<h1>'),1)
 ...
 def test_gateway_is_discoverable(self):
  self.assertEqual(self.sitemap.count('https://healthrenewal.org/assessments/global-measures/'),1)
