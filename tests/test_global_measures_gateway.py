...
class GlobalMeasuresGatewayTest(unittest.TestCase):
 ...
 def test_gateway_count_matches_machine_catalog(self):
  m=re.search(r'<strong>(\d+) أداة/بطارية فعلية حاليًا</strong>',self.html);self.assertIsNotNone(m);self.assertEqual(int(m.group(1)),self.catalog['actual_tool_count']);self.assertEqual(self.catalog['actual_tool_count'],34);self.assertEqual(len(self.catalog['tools']),34);self.assertIn('Functional Ambulation Category',self.html);self.assertIn('Modified Tardieu Scale',self.html);self.assertIn('Modified Rankin Scale',self.html);self.assertIn('22 أداة/بطارية',self.html)
 def test_rmd_rights_model_and_fac_lifecycle_are_explicit(self):
  self.assertIn('RMD لا يملك الأدوات',self.html);self.assertIn('Cost + Cost Description',self.html);self.assertIn('operational-sheet-only',self.html);self.assertIn('rule_7',self.audit['method']);fac=next(x for x in self.audit['current_library'] if x['id']=='fac');self.assertEqual(fac['decision'],'operational-sheet-only');self.assertEqual(fac['rmd_cost'],'Free');self.assertIn('no authoritative explicit Public Domain',fac['rights_source']);self.assertIn('Operational Arabic paraphrase',fac['arabic_state']);self.assertNotIn('fac',{x['id'] for x in self.audit['priority_candidates']})
