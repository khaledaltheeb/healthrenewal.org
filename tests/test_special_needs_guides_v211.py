...
MANIFEST = ROOT / "content" / "v211" / "special-needs-guides-manifest-ar.json"
GUIDE_DIR = ROOT / "content" / "v211" / "special-needs-guides"
PUBLISHER = ROOT / "scripts" / "publish_special_needs_guides_v211.py"
...
class SpecialNeedsGuidesV211Tests(unittest.TestCase):
    def load(self):
    ...
    def make_site(self, *, sitemap_index: bool = False) -> Path:
        site = Path(tempfile.mkdtemp(prefix="special-needs-v211-"))
    ...
    def run_publisher(self, site: Path) -> dict:
        ...
        return json.loads((site / "api/special-needs-guides-v211.json").read_text(encoding="utf-8"))

    def test_sources_are_complete_deep_and_authoritative(self) -> None:
        manifest, guides = self.load()
        self.assertEqual(manifest["version"], 211)
    ...
    def test_publishes_five_searchable_pages_and_is_idempotent(self) -> None:
        ...
        self.assertEqual(hub.count("<!-- special-needs-guides-v211:start -->"), 1)
        self.assertEqual(hub.count("<!-- special-needs-guides-v211:end -->"), 1)
        ...
        self.assertEqual(len(v211_locs), 5)
        self.assertEqual(len(v211_locs), len(set(v211_locs)))
