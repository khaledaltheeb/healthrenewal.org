...
def test_workflow_is_complete_and_measurable():
    ...
    assert [item["step"] for item in workflow] == list(range(1, 8))
...
def test_continuity_safeguards_are_explicit():
    data = load_data()
...
def test_professional_limits_and_no_false_endorsement():
    ...
    for phrase in ["معتمد من منظمة الصحة العالمية", "بشراكة مع الأمم المتحدة", "مصدق رسميًا"]:
...
def test_internal_links_faq_and_language():
    data = load_data()
