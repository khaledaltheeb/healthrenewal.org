...
MANIFEST = ROOT / "content" / "v212" / "special-needs-guides-manifest-ar.json"
GUIDE_DIR = ROOT / "content" / "v212" / "special-needs-guides"
PUBLISHER = ROOT / "scripts" / "publish_special_needs_guides_v212.py"
...
PREVIOUS_MANIFESTS = [
    ROOT / "content" / f"v{version}" / "special-needs-guides-manifest-ar.json"
    for version in (209, 210, 211)
]
...
class SpecialNeedsGuidesV212Tests(unittest.TestCase):
    def load(self):
    ...
    def make_site(self, *, sitemap_index: bool = False) -> Path:
        site = Path(tempfile.mkdtemp(prefix="special-needs-v212-"))
    ...
    def run_publisher(self, site: Path) -> dict:
        ...
        return json.loads((site / "api/special-needs-guides-v212.json").read_text(encoding="utf-8"))

    def test_sources_depth_review_state_and_unique_slugs(self) -> None:
        manifest, guides = self.load()
        self.assertEqual(manifest["version"], 212)
        ...
        for path in PREVIOUS_MANIFESTS:
            previous_slugs.update(json.loads(path.read_text(encoding="utf-8"))["guide_slugs"])
        self.assertTrue(set(guides).isdisjoint(previous_slugs))
        ...
        for slug, guide in guides.items():
            ...
            self.assertGreaterEqual(len(guide["intro"]), 3)
            ...
            self.assertGreaterEqual(source_words, 900, (slug, source_words))
    ...
    def test_publishes_five_searchable_pages_and_is_idempotent(self) -> None:
        ...
        self.assertGreaterEqual(first["minimum_source_words"], 900)
        self.assertEqual(first["review_status"], "internally-reviewed")
        self.assertEqual(first["external_review"], "recommended-not-completed")
        hub = (site / "special-needs/index.html").read_text(encoding="utf-8")
        self.assertEqual(hub.count("<!-- special-needs-guides-v212:start -->"), 1)
        self.assertEqual(hub.count("<!-- special-needs-guides-v212:end -->"), 1)
        for slug in json.loads(MANIFEST.read_text(encoding="utf-8"))["guide_slugs"]:
            ...
            self.assertGreaterEqual(len(visible_words), 1000, (slug, len(visible_words)))
        tree = ET.parse(site / "sitemap-special-needs.xml")
        locs = [node.text for node in tree.findall("sm:url/sm:loc", NS)]
        v212_locs = [
            loc for loc in locs
            if loc and any(slug in loc for slug in json.loads(MANIFEST.read_text(encoding="utf-8"))["guide_slugs"])
        ]
        self.assertEqual(len(v212_locs), 5)
        self.assertEqual(len(v212_locs), len(set(v212_locs)))
