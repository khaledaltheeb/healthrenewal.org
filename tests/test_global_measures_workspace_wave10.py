...
class WorkspaceWave10Test(unittest.TestCase):
 ...
 def test_catalog_current_count_and_unique_routes(self):
  ...
  ids={x['id'] for x in tools};self.assertTrue({'gas','healthy-days','bbs','bbt','9hpt','arat','mas','mts','mrs','fac'}.issubset(ids));gas=next(x for x in tools if x['id']=='gas');self.assertIn('-2 to +2',gas['score']);self.assertIn('optional',gas['score']);self.assertEqual(gas['rights_state'],'rmd-free-operational-template-no-third-party-form-reproduced');arat=next(x for x in tools if x['id']=='arat');self.assertIn('0-57 per arm',arat['score']);self.assertIn('no bilateral total',arat['score'])
 ...
 def test_workspace_and_finder_are_rtl_indexable_unique(self):
  ...
  self.assertIn('38 أداة/بطارية فعلية',self.work);self.assertIn('38 actual tools',self.finder);self.assertIn('ARAT',self.work);self.assertIn('ARAT',self.finder);self.assertIn('GAS',self.work);self.assertIn('GAS',self.finder)
