...
class WorkspaceWave10Test(unittest.TestCase):
 ...
 def test_catalog_current_count_and_unique_routes(self):
  ...
  ids={x['id'] for x in tools};self.assertTrue({'healthy-days','bbs','bbt','9hpt','mas','mts','mrs','fac'}.issubset(ids));hpt=next(x for x in tools if x['id']=='9hpt');self.assertIn('no bilateral total',hpt['score']);self.assertEqual(hpt['rights_state'],'rmd-not-free-equipment-operational-sheet-rights-not-public-domain-verified')
