...
class WorkspaceWave10Test(unittest.TestCase):
 ...
 def test_catalog_current_count_and_unique_routes(self):
  tools=self.catalog['tools'];n=self.catalog['actual_tool_count'];self.assertEqual(n,37);self.assertEqual(len(tools),37);self.assertEqual(len({x['id'] for x in tools}),37);self.assertEqual(len({x['route'] for x in tools}),37)
  ids={x['id'] for x in tools};self.assertTrue({'gas','healthy-days','bbs','bbt','9hpt','mas','mts','mrs','fac'}.issubset(ids));gas=next(x for x in tools if x['id']=='gas');self.assertIn('-2 to +2',gas['score']);self.assertIn('optional',gas['score']);self.assertEqual(gas['rights_state'],'rmd-free-operational-template-no-third-party-form-reproduced')
 ...
 def test_workspace_and_finder_are_rtl_indexable_unique(self):
  ...
  self.assertIn('37 أداة/بطارية فعلية',self.work);self.assertIn('37 actual tools',self.finder);self.assertIn('GAS',self.work);self.assertIn('GAS',self.finder)
 ...
 def test_packets_retain_redundancy_and_goal_guards(self):
  for text in ('قاعدة تقليل العبء','حزمة الأهداف الفردية والنتائج الشخصية','GAS — تحقيق الهدف −2…+2','PSFS — قدرة النشاط 0–10','لا تجمع GAS + PSFS في مجموع واحد','حزمة الطرف العلوي والبراعة اليدوية','9HPT — البراعة الدقيقة للأصابع','MTS — R1/R2'):self.assertIn(text,self.packets)
