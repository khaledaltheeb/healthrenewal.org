...
class WorkspaceWave10Test(unittest.TestCase):
 ...
 def test_catalog_current_count_and_unique_routes(self):
  tools=self.catalog['tools'];n=self.catalog['actual_tool_count'];self.assertEqual(n,36);self.assertEqual(len(tools),36);self.assertEqual(len({x['id'] for x in tools}),36);self.assertEqual(len({x['route'] for x in tools}),36)
  ids={x['id'] for x in tools};self.assertTrue({'healthy-days','bbs','bbt','9hpt','mas','mts','mrs','fac'}.issubset(ids));bbt=next(x for x in tools if x['id']=='bbt');hpt=next(x for x in tools if x['id']=='9hpt');self.assertIn('no bilateral total',bbt['score']);self.assertIn('no bilateral total',hpt['score']);self.assertEqual(hpt['rights_state'],'rmd-not-free-equipment-operational-sheet-rights-not-public-domain-verified')
 ...
 def test_workspace_and_finder_are_rtl_indexable_unique(self):
  ...
  self.assertIn('36 أداة/بطارية فعلية',self.work);self.assertIn('36 actual tools',self.finder);self.assertIn('9HPT',self.work);self.assertIn('9HPT',self.finder)
 ...
 def test_packets_retain_redundancy_and_hand_function_guards(self):
  for text in ('قاعدة تقليل العبء','FAC — استقلالية المشي','Not Testable','حزمة الطرف العلوي والبراعة اليدوية','BBT — البراعة اليدوية الإجمالية','9HPT — البراعة الدقيقة للأصابع','Grip Strength — القوة','لا تعتبر القوة والبراعة الإجمالية والبراعة الدقيقة الشيء نفسه','MTS — R1/R2','لا تجمع درجات المواد المختلفة'):self.assertIn(text,self.packets)
