...
def test_wave_006_uses_supported_sources_and_categories():
...
def test_wave_006_does_not_collide_with_previous_catalog():
...
def test_wave_006_installs_full_contract():
    report=wave.install(implementation)
    assert report["added_guides"]==50; assert report["cumulative_wave_guides"]==300; assert report["expected_institutional_guides"]==387; assert report["expected_source_guides"]==401; assert report["minimum_published_guides"]==400; assert report["unique_slugs"]==50; assert report["unique_titles"]==50; assert report["minimum_sources"]>=2; assert report["specialist_review_claimed"] is False; assert catalog.EXPECTED_GUIDES==387; assert len(catalog.institutional_guides())==387

def test_wave_006_topics_have_substantive_fields():
