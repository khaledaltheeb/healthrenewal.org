...
class ModifiedTardieuWave16Test(unittest.TestCase):
 ...
 def test_rights_are_conservative_not_public_domain_claim(self):
  self.assertEqual(self.reg['rmd']['cost'],'Free');self.assertFalse(self.reg['rights']['instrument_public_domain_verified']);self.assertEqual(self.reg['rights']['rawafid_decision'],'operational-sheet-only');row=next(x for x in self.audit['current_library'] if x['id']=='mts');self.assertEqual(row['decision'],'operational-sheet-only')
 def test_catalog_library_packet_and_sitemap_preserve_mts(self):
  self.assertGreaterEqual(self.catalog['actual_tool_count'],33);row=next(x for x in self.catalog['tools'] if x['id']=='mts');self.assertEqual(row['route'],'/sectors/rehabilitation/measures/modified-tardieu-scale/');self.assertIn("mts='/sectors/rehabilitation/measures/modified-tardieu-scale/'",self.lib);self.assertIn('MTS — R1/R2',self.packets);urls=[e.text for e in ET.fromstring(self.xml).findall('s:url/s:loc',NS)];self.assertEqual(urls.count('https://healthrenewal.org/sectors/rehabilitation/measures/modified-tardieu-scale/'),1)
 def test_future_candidate_gates_are_conservative(self):
  rows={x['id']:x for x in self.audit['priority_candidates']};self.assertEqual(rows['fma']['decision'],'official-link-only');self.assertEqual(rows['rmdq24']['decision'],'hold-exact-arabic');self.assertNotIn('fac',rows)
