...
import care_guides_wave_v403 as wave
import publish_care_guides_v246 as implementation


def test_wave_004_has_exactly_fifty_unique_topics():
    topics = wave.topics()
...
def test_wave_004_does_not_collide_with_previous_catalog():
...
def test_wave_004_installs_full_contract():
    report = wave.install(implementation)
    assert report["added_guides"] == 50
    assert report["cumulative_wave_guides"] == 200
    assert report["expected_institutional_guides"] == 287
    assert report["expected_source_guides"] == 301
    assert report["minimum_published_guides"] == 300
    assert report["unique_slugs"] == 50
    assert report["unique_titles"] == 50
    assert report["minimum_sources"] >= 2
    assert report["specialist_review_claimed"] is False
    assert catalog.EXPECTED_GUIDES == 287
    assert len(catalog.institutional_guides()) == 287


def test_wave_004_topics_have_substantive_fields():
