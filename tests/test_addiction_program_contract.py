...
def test_governance_contracts_are_linked_and_exist():
    ...
    for field in (
        ...
        target = ROOT / index[field]
        assert target.is_file(), f"Missing {field}: {target}"
...
def test_structured_source_registry_is_reviewable():
    ...
    today = date.fromisoformat(registry["verified_on"])
    authorities = set()
    for source in sources:
        assert required_fields.issubset(source), source.get("id")
        assert source["url"].startswith("https://")
        assert date.fromisoformat(source["verified_on"]) <= today
        assert date.fromisoformat(source["next_review_on"]) > today
    ...
    assert required_authorities.issubset(authorities)
