...
def test_source_record_exists_and_uses_single_canonical_slug():
    ...
    assert record["publication_requirements"]["canonical_path"] == "/special-needs/cornelia-de-lange-syndrome/"
...
def test_review_status_and_publication_status_are_honest():
    ...
    assert status["publication_status"] == "source-governance-only"
...
def test_future_publication_contract_requires_depth_schema_and_accessibility():
    ...
    assert {"MedicalWebPage", "MedicalCondition", "BreadcrumbList"} <= set(requirements["required_schema"])
