...
(() => {
  const RELEASE = "2026.07.25-progress-plan.5";
  const PLAN_SCHEMA = "pa-original-progress-plan-v3";
  const ASSESSMENT_PURPOSE = "progress_monitoring";
  const MAX_RETAINED_AUDIT_EVENTS = 200;
  const AUDIT_BOUNDARY = "local-change-history-not-tamper-evident-not-legal-record";
  ...
  const assessmentOptions = (caseId) => {
    ...
    const ids = [...new Set(
      (record?.sessions || []).map((item) => item.assessmentId).filter(Boolean)
    )];
    return ids.map((id) => ({
      id,
      title: window.PA_DEMO_DATA?.explorers?.find((item) => item.id === id)?.title || id,
    }));
  };

  const directionLabels = {
    increase: "زيادة الإشارة الوصفية",
    decrease: "انخفاض الإشارة الوصفية",
    stable: "ثبات الإشارة الوصفية",
    observe: "مراقبة دون اتجاه مسبق",
  };
  ...
  const reviewStatus = (plan, series) => {
    ...
    if (!series || series.sessions.length < 2) {
      return { code: "baseline_only", label: "بانتظار قياس متابعة" };
    }
    if (!series.comparability?.interpretable) {
      return { code: "context_blocked", label: "المراجعة محجوبة لاختلاف السياق" };
    }
    if (due && due > today) {
      return { code: "scheduled", label: "المقارنة متاحة والموعد لاحق" };
    }
  ...
  const validatePlan = (data, record, previous) => {
    ...
    if (!allowedAssessments.has(data.assessmentId)) {
      throw new Error("assessment_not_in_case");
    }
    if (previous && previous.assessmentId !== data.assessmentId) {
      throw new Error("assessment_change_forbidden");
    }
    if (!directionLabels[data.targetDirection]) {
      throw new Error("invalid_direction");
    }
    if (
      !/^\d{4}-\d{2}-\d{2}$/.test(data.reviewDate || "")
      || !Number.isFinite(Date.parse(`${data.reviewDate}T00:00:00`))
    ) {
    ...
    if (required.some((key) => clean(data[key]).length < 8)) {
      throw new Error("documentation_required");
    }
    if (previous && clean(data.editReason).length < 5) {
      throw new Error("edit_reason_required");
    }
  };
  ...
  const savePlan = (data) => {
    ...
    if (!store || !identity?.uid || !record) {
      throw new Error("active_case_required");
    }

    const plans = Array.isArray(record.originalProgressPlans)
      ? record.originalProgressPlans
      : [];
    ...
    const event = previous
      ? {
          ...
          reason: clean(data.editReason),
          previous: snapshotFrom(previous),
          next: snapshot,
        }
      : {
          event: "plan_created",
    ...
    const previousAudit = Array.isArray(previous?.auditTrail)
      ? previous.auditTrail
      : [];
    const previousTotal = Number.isInteger(previous?.auditEventCount)
      ? previous.auditEventCount
      : previousAudit.length;
    const auditEventCount = previousTotal + 1;
    const auditTrail = [...previousAudit, event].slice(-MAX_RETAINED_AUDIT_EVENTS);
    ...
    const next = {
      ...
      createdByUid: previous?.createdByUid || identity.uid,
      updatedAt: now,
      updatedByUid: identity.uid,
      auditBoundary: AUDIT_BOUNDARY,
      auditEventCount,
      auditRetentionLimit: MAX_RETAINED_AUDIT_EVENTS,
      auditTrail,
    };

    record.originalProgressPlans = [
      next,
      ...plans.filter((item) => item.planId !== id),
    ];
    record.updatedAt = now;
    progress().persistStore(store);
    window.dispatchEvent(new CustomEvent(
      "pa-original-progress-plan-saved",
      { detail: { caseId: data.caseId, planId: id } }
    ));
    return next;
  };
  ...
  const exportPlans = (caseId) => {
    ...
    const payload = {
      schema: "pa-original-progress-plan-export-v3",
      release: RELEASE,
    ...
    anchor.href = url;
    anchor.download = `${caseId}-original-progress-plans.json`;
    anchor.click();
    setTimeout(() => URL.revokeObjectURL(url), 500);
  };

  const formHtml = (caseId, options, existing = null) => {
    const allowedOptions = existing
      ? options.filter((item) => item.id === existing.assessmentId)
      : options;
    const displayOptions = allowedOptions.length
      ? allowedOptions
      : [{
          id: existing?.assessmentId || "",
          title: existing?.assessmentId || "—",
        }];
    return `<form class="progress-plan-form" data-progress-plan-form="${esc(caseId)}">
      ...
      <p class="muted">لا تعلن المنصة تحقق الهدف أو التحسن السريري آليًا. اختلاف السياق أو المجيب أو طريقة التطبيق قد يمنع المقارنة، ولا تُحوّل الإشارة الوصفية إلى درجة معيارية أو تشخيص.</p>
      <p class="muted">سجل التغييرات محلي وقابل للنسخ أو الحذف من صاحب الجهاز؛ ليس سجلًا قانونيًا غير قابل للتلاعب. تحتفظ الخطة بآخر ${MAX_RETAINED_AUDIT_EVENTS} حدث مع عدّاد إجمالي للأحداث.</p>
      <p class="muted" data-progress-plan-status aria-live="polite"></p>
    </form>`;
  };

  const renderPlans = (caseId) => {
    const panel = document.querySelector(
      `[data-original-progress="${CSS.escape(caseId)}"]`
    );
    if (!panel) return;
    let host = panel.querySelector("[data-original-progress-plans]");
    if (!host) {
      host = document.createElement("section");
      host.className = "progress-plan-section";
      host.dataset.originalProgressPlans = caseId;
    ...
    const cards = plans.length
      ? plans.map((plan) => {
          ...
          const title = options.find(
            (item) => item.id === plan.assessmentId
          )?.title || plan.assessmentId;
          const retainedEvents = Array.isArray(plan.auditTrail)
            ? plan.auditTrail.length
            : 0;
          const totalEvents = Number.isInteger(plan.auditEventCount)
            ? plan.auditEventCount
            : retainedEvents;
          return `<article class="progress-plan-card"><div><strong>${esc(title)}</strong><span class="comparability-badge ${esc(status.code)}">${esc(status.label)}</span></div><p><b>الغرض:</b> متابعة التقدم</p><p><b>الهدف:</b> ${esc(plan.functionalGoal)}</p><p><b>أولوية الأسرة أو الشخص:</b> ${esc(plan.familyPriority || "غير موثقة")}</p><p><b>ملاحظة مقدم الخدمة:</b> ${esc(plan.providerObservation || "غير موثقة")}</p><p><b>سياق القياس:</b> ${esc(plan.measurementContext || "غير موثق")}</p><p><b>الاتجاه:</b> ${esc(directionLabels[plan.targetDirection] || plan.targetDirection)}</p><p><b>المراجعة:</b> ${esc(plan.reviewDate)} — ${esc(plan.reviewOwner || "لم يحدد المسؤول")}</p><p><b>قاعدة القرار:</b> ${esc(plan.decisionRule)}</p><p><b>حدود التفسير:</b> ${esc(plan.interpretationLimit || "غير موثقة")}</p><p class="muted">إجمالي أحداث السجل المحلي: ${totalEvents} — المحتفظ بها: ${retainedEvents}</p><button class="button ghost small-button" type="button" data-edit-progress-plan="${esc(plan.planId)}">تعديل موثق</button></article>`;
        }).join("")
      : '<p class="muted">لا توجد خطة هدف موثقة لهذه الحالة بعد.</p>';
    host.innerHTML = `<div class="section-heading compact"><div><h4>خطط الأهداف والمراجعة</h4><p class="muted">تربط متابعة التقدم بهدف وظيفي وأولوية الأسرة وملاحظة مقدم الخدمة وسياق قابل للمقارنة، وتدخل ضمن نسخة الحالة الاحتياطية المحلية.</p></div><button class="button ghost small-button" type="button" data-export-progress-plans="${esc(caseId)}">تصدير الخطط</button></div><div class="progress-plan-grid">${cards}</div>${options.length ? formHtml(caseId, options) : '<p>أضف جلسة أداة أصلية أولًا.</p>'}`;
  };
  ...
  document.addEventListener("submit", (event) => {
    ...
    try {
      savePlan({
        caseId: form.dataset.progressPlanForm,
        ...Object.fromEntries(new FormData(form)),
      });
      renderPlans(form.dataset.progressPlanForm);
    } catch (error) {
      const status = form.querySelector("[data-progress-plan-status]");
      if (!status) return;
      status.textContent = error.message === "edit_reason_required"
        ? "سبب التعديل إلزامي."
        : error.message === "documentation_required"
          ? "أكمل أولوية الأسرة وملاحظة مقدم الخدمة وسياق القياس ومسؤول المراجعة وحدود التفسير."
          : "تحقق من الأداة والتاريخ والحقول المطلوبة.";
    }
  });
  ...
  document.addEventListener("click", (event) => {
    ...
    if (exportButton) {
      exportPlans(exportButton.dataset.exportProgressPlans);
      return;
    }
    ...
    const plan = caseId
      ? getPlans(caseId).find(
          (item) => item.planId === button.dataset.editProgressPlan
        )
      : null;
    if (!plan || !host) return;
    ...
    wrapper.innerHTML = formHtml(
      plan.caseId,
      assessmentOptions(plan.caseId),
      plan
    );
    host.querySelector("[data-progress-plan-form]")
      ?.replaceWith(wrapper.firstElementChild);
  });

  const injectNew = () => document
    .querySelectorAll("[data-original-progress]")
    .forEach((panel) => {
      if (!panel.querySelector("[data-original-progress-plans]")) {
        renderPlans(panel.dataset.originalProgress);
      }
    });
  const refreshAll = () => document
    .querySelectorAll("[data-original-progress]")
    .forEach((panel) => renderPlans(panel.dataset.originalProgress));

  new MutationObserver(injectNew).observe(document.body, {
    childList: true,
    subtree: true,
  });
  ...
  window.PA_ORIGINAL_PROGRESS_PLANS = {
    release: RELEASE,
    schema: PLAN_SCHEMA,
    assessmentPurpose: ASSESSMENT_PURPOSE,
    auditBoundary: AUDIT_BOUNDARY,
