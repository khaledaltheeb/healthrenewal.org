...
(() => {
  ...
  const LICENSE_BOUNDARY = "original-license-safe-tools-only";
  const esc = (value) => String(value ?? "").replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#039;");
  const clean = (value) => String(value || "").trim();
  const progress = () => window.PA_ORIGINAL_PROGRESS;
  const originalTools = () => new Map((window.PA_DEMO_DATA?.explorers || []).map((item) => [item.id, item]));
  ...
  const assessmentOptions = (caseId) => {
    ...
    const ids = [...new Set((record?.sessions || []).map((item) => item.assessmentId).filter((id) => tools.has(id)))];
    return ids.map((id) => ({ id, title: tools.get(id)?.title || id }));
  };
  ...
  const validatePlan = (data, record, previous) => {
    ...
    if (!originalTools().has(data.assessmentId)) throw new Error("protected_or_unknown_assessment_forbidden");
    ...
    if (previous && JSON.stringify(snapshotFrom(previous)) === JSON.stringify(snapshotFrom(data))) throw new Error("no_documented_change");
  };
  const savePlan = (data) => {
    ...
    const actor = { actorUid: identity.uid, actorRole: identity.role || "visitor" };
    const event = previous
      ? { event: "plan_revised", at: now, ...actor, reason: clean(data.editReason), previous: snapshotFrom(previous), next: snapshot }
      : { event: "plan_created", at: now, ...actor, next: snapshot };
    const next = {
      schema: PLAN_SCHEMA, release: RELEASE, planId: id, caseId: data.caseId, assessmentId: data.assessmentId, ...snapshot,
      createdAt: previous?.createdAt || now, createdByUid: previous?.createdByUid || identity.uid, createdByRole: previous?.createdByRole || identity.role || "visitor",
      updatedAt: now, updatedByUid: identity.uid, updatedByRole: identity.role || "visitor",
      auditTrail: [...(previous?.auditTrail || []), event]
    };
  ...
  const exportPlans = (caseId) => {
    ...
    const payload = {
      ...
      generatedAt: new Date().toISOString(), assessmentPurpose: ASSESSMENT_PURPOSE, licenseBoundary: LICENSE_BOUNDARY,
  ...
  const formHtml = (caseId, options, existing = null) => {
    ...
    return `<form class="progress-plan-form" data-progress-plan-form="${esc(caseId)}">
      <div class="notice purpose-boundary"><strong>نوع الاستخدام: متابعة التقدم.</strong> ليست هذه الخطة مسحًا أو فرزًا، ولا تقييمًا تشخيصيًا، ولا تقييمًا وظيفيًا مستقلًا. تقتصر على الأدوات الأصلية الآمنة ترخيصيًا، وتُفسر مع سؤال الإحالة والتقييم الوظيفي ومصادر الأسرة ومقدم الخدمة.</div>
  ...
  document.addEventListener("submit", (event) => {
    ...
    try { savePlan({ caseId: form.dataset.progressPlanForm, ...Object.fromEntries(new FormData(form)) }); renderPlans(form.dataset.progressPlanForm); }
      ...
      if (!status) return;
      const messages = {
        edit_reason_required: "سبب التعديل إلزامي.",
        documentation_required: "أكمل أولوية الأسرة وملاحظة مقدم الخدمة وسياق القياس ومسؤول المراجعة وحدود التفسير.",
        protected_or_unknown_assessment_forbidden: "خطط المتابعة مخصصة للأدوات الأصلية الآمنة ترخيصيًا فقط.",
        no_documented_change: "لم يتغير محتوى الخطة؛ عدّل حقلًا مهنيًا قبل إنشاء حدث مراجعة جديد."
      };
      status.textContent = messages[error.message] || "تحقق من الأداة والتاريخ والحقول المطلوبة.";
    }
  });
  ...
  window.PA_ORIGINAL_PROGRESS_PLANS = { release: RELEASE, schema: PLAN_SCHEMA, assessmentPurpose: ASSESSMENT_PURPOSE, licenseBoundary: LICENSE_BOUNDARY, getPlans, savePlan, reviewStatus, exportPlans, validatePlan };
})();
