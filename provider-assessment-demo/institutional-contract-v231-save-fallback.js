...
(() => {
  ...
  const makeId = () => `PRO-${crypto?.randomUUID?.().replaceAll("-", "").slice(0, 16).toUpperCase() || `${Date.now().toString(36)}${Math.random().toString(36).slice(2, 9)}`.toUpperCase()}`;
  ...
  window.PA_V231_SAVE_FALLBACK = { release: RELEASE, baseRecordIsValid };
})();