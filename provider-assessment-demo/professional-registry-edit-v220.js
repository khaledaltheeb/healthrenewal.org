...
(() => {
  ...
  const toolFor = (record) => {
    ...
    const contract = registry.customContractForMode(record.administrationMode);
    return {
  ...
  const structured = (form, record, tool, now) => {
    const values = {};
    for (const name of maturityFields) {
  ...
  const protectedText = (form) => [
    form.elements.scoreReference.value,
    form.elements.notes.value,
    ...maturityFields.map((name) => editInput(form, name)?.value || ""),
  ].join(" ");
  ...
  function saveStructuredEdit(event) {
    ...
    const completed = COMPLETED.has(record.recordStatus);
    const rights = editInput(form, "rightsBasis").value;
    if ((completed && rights === "pending_review") || (rights !== "pending_review" && !tool.professionalContract.permittedRightsBases.includes(rights))) {
      event.preventDefault();
      event.stopImmediatePropagation();
    ...
    if (PROTECTED_TEXT.test(protectedText(form))) {
      event.preventDefault();
      event.stopImmediatePropagation();
    ...
    const nextBase = Object.fromEntries(baseFields.map((field) => [field, clean(form.elements[field]?.value, field === "notes" ? 3000 : 300)]));
    const nextMaturity = structured(form, record, tool, now);
    const changes = [];
    for (const field of baseFields) {
      const previous = String(record[field] || "");
      if (nextBase[field] !== previous) changes.push({ field, from: previous, to: nextBase[field] });
    }
    if (JSON.stringify(record.professionalMaturity || null) !== JSON.stringify(nextMaturity)) {
      changes.push({ field: "professionalMaturity", from: record.professionalContractVersion || "legacy", to: VERSION });
    }
    ...
    const persisted = typeof set === "function" && typeof storeKey === "function"
      ...
      : (save(), true);
    if (!persisted) {
      restoreObject(record, recordSnapshot);
