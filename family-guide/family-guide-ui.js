...
function renderCondition(){
 ...
 ${section('urgent','10','متى نطلب مساعدة عاجلة؟',list(c.urgent),'warning')}
...
function renderIndex(){
 ...
 if(input) input.addEventListener('input',()=>{const q=input.value.trim().toLowerCase();let visible=0;root.querySelectorAll('[data-search]').forEach(el=>{const show=!q||el.dataset.search.includes(q);el.hidden=!show;if(show)visible++;});empty.style.display=visible?'none':'block';});
}
